...
if (-not $python -and (Get-Command uv -ErrorAction SilentlyContinue)) { $python = uv python find 3.12 2>$null }
if (-not $python -and (Get-Command py -ErrorAction SilentlyContinue)) { $python = & py -3.12 -c "import sys;print(sys.executable)" 2>$null }
