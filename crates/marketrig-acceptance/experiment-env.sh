...
if [[ -z "${MARKETRIG_EXPERIMENT_MEMORY_API_KEY:-}" ]]; then
  read -rs -p "provider API key (not echoed): " MARKETRIG_EXPERIMENT_MEMORY_API_KEY
  echo >&2
fi
...
export MARKETRIG_EXPERIMENT_MEMORY_BASE_URL="${MARKETRIG_EXPERIMENT_MEMORY_BASE_URL:-https://api.openai.com/v1}"
export MARKETRIG_EXPERIMENT_MEMORY_API_KEY
export MARKETRIG_EXPERIMENT_MEMORY_LLM_MODEL="${MARKETRIG_EXPERIMENT_MEMORY_LLM_MODEL:-gpt-4.1-mini}"
export MARKETRIG_EXPERIMENT_MEMORY_EMBEDDING_MODEL="${MARKETRIG_EXPERIMENT_MEMORY_EMBEDDING_MODEL:-text-embedding-3-small}"
