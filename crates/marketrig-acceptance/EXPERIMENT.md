...
The experiment is the operator-attended half of the acceptance chain (root `sdd/SPEC.md` §17; R1 feature SPEC §10.3, R2 feature SPEC §10.3). Slice 002's exit needs E1 (Codex CLI) and E2 (Claude Code) once per platform-and-runtime cell; slice 003 adds E3, the scheduled-trigger scenario, and slice 004 adds E4, the runtime-delivery scenario, to the same cells. E5 was Hindsight's and went with it; slice 011 adds E6, the OpenViking memory-and-skills scenario (`sdd/features/openviking-continuity/SPEC.md` §7.3), slice 013 adds E7, the A-share scenario on the real HiThink service (`sdd/features/hithink-a-share/SPEC.md` §6.3), and slice 015 adds E8, the trigger-invocation scenario (`sdd/features/event-triggers/SPEC.md` §7.2), which runs last in the cell. E6 and E7 each skip with evidence unless their own prerequisites are named (§1, §7, §8); E8 needs nothing beyond the cell's runtime. The R1 macOS cells ran on 2026-09-02 (`target/acceptance/experiment-codex-1788316954/` and `experiment-claude-1788317581/`); this guide is for the two Windows cells and doubles as the procedure anywhere. Commands are PowerShell.

One invocation per cell runs **six** scenarios back to back — E1 or E2, E3, E4, E6, E7, and E8 — each on its own daemon, desk, and bundle. They share your terminal and your hands, so the harness serializes them: each scenario's instructions print only once the previous one has finished. E4, E6, E7, and E8 are unlike the first two: MarketRig launches the runtime itself, with the adapter already registered, and **your console becomes the desk's terminal** for the whole scenario.
...
- E7's own prerequisite (`sdd/features/hithink-a-share/SPEC.md` §1.2, §6.3). Without it E7 skips with evidence and the rest of the cell still runs:
  - `MARKETRIG_EXPERIMENT_HITHINK_API_KEY` — a real HiThink key, the one you would type into Settings. The harness saves it through `PUT /research/hithink`, which validates it with one real request against the service, so a wrong key fails the cell at that call. It is never printed and never written to the bundle: the cell's last steps remove it from the credential store and grep every file of the bundle for it.

    The dot-sourced script below does not carry it — set it in the same window, unechoed, before the run:

    ```powershell
    $env:MARKETRIG_EXPERIMENT_HITHINK_API_KEY = [System.Net.NetworkCredential]::new("", (Read-Host -AsSecureString "HiThink key")).Password
    ```

    ```bash
    read -rs MARKETRIG_EXPERIMENT_HITHINK_API_KEY && export MARKETRIG_EXPERIMENT_HITHINK_API_KEY   # macOS
    ```
- One dot-sourced script selects the cell and exports all seven of E6's — `. crates\marketrig-acceptance\experiment-env.ps1 codex|claude` in PowerShell, `source crates/marketrig-acceptance/experiment-env.sh codex|claude` on macOS. It prompts for the provider key without echoing it, and leaves E6's variables unset (so E6 skips) when the interpreters or the wheel directory are not there.
...
- Nothing else. Do **not** set `MARKETRIG_TEST_DATA_ROOT`, `MARKETRIG_TEST_NO_TRADING`, `MARKETRIG_TEST_QUOTE_URL`, or `MARKETRIG_TEST_HITHINK_URL` yourself: the harness relocates the root into the bundle and clears both feed seams so the daemon polls real Yahoo, and E7 fails on the spot if a stand-in HiThink URL is on its environment.
- Pick the time. Before asking you to do anything, the harness waits up to 60 seconds for one `LIVE` observation from Yahoo, so at least one catalog market must be in session (feature SPEC §2.2, Monday–Friday, no holiday calendar):
  ...
  E7 reads and trades `600519.XSHG` and needs no window at all: it stages both of its Shanghai days through the controlled clock (§8), so it runs after the Shanghai close, on a weekend, or on a holiday just as well as mid-session. Outside real trading hours the snapshot is the day's last and does not move, which is why its two legs are MARKET orders. The one thing to watch is the `Day:` line of its printout: if it says `NOT EXECUTABLE`, stop (§11).

## 2. Start the cell

One cell per invocation; the other cell's tests skip. The harness builds `marketrigd`, `marketrig`, `marketrig-mcp`, and the `trigger-code` helper itself, spawns a daemon, creates a run-stamped desk, and prints its instructions. It then waits up to 15 minutes per step on the daemon's durable rows. Budget 110–150 minutes for a cell: E1/E2, then E3, E4, E6, E7, and E8 — E6 alone spends several minutes provisioning its own Python environment from the wheels and starting the real OpenViking server before it asks you for anything.
...
The bundles are `target\acceptance\experiment-<cell>-<stamp>\` for E1/E2, `target\acceptance\experiment-e3-<cell>-<stamp>\` for E3, `target\acceptance\experiment-e4-<cell>-<stamp>\` for E4, `target\acceptance\experiment-e6-<cell>-<stamp>\` for E6, `target\acceptance\experiment-e7-<cell>-<stamp>\` for E7, and `target\acceptance\experiment-e8-<cell>-<stamp>\` for E8. Do **not** set `MARKETRIG_ACCEPTANCE_OUT` for a cell: it would point every scenario at one directory and each would overwrite the last one's `observations.jsonl`. Copy the `Desk:`, `Data root:`, and `Adapter:` lines from each printout; the same text is in `<bundle>\instructions.txt` (E1/E2), `<bundle>\instructions-e3.txt` (E3), `<bundle>\instructions-e4.txt` (E4), `<bundle>\instructions-e6.txt` (E6), `<bundle>\instructions-e7.txt` plus `instructions-e7-sell.txt` (E7), and `<bundle>\instructions-e8.txt` (E8). Leave this window running and open a second one for the session. E6's bundle is the largest by far: the data root *is* the bundle, so it carries the whole provisioned virtual environment as well as the plugin's own state and logs.
...
2. sent `PUT /openviking/setup` with your Python, Node, and wheel directory and waited for the row to leave `PROVISIONING` — `uv venv`, then `uv pip install --offline --no-index openviking==0.4.17.1` with the bundled uv, under two minutes on a cold directory (pip took 13–17 on Windows, where every unpacked file meets Defender; the harness allows 30) — and then for the child to answer `/ready`;
...
## 8. Drive the session — E7

E7 is slice 013's scenario (`sdd/features/hithink-a-share/SPEC.md` §6.3): **the real HiThink service**, as this desk's A-share data and as its `CN` feed. It runs on its own daemon and desk, and skips with evidence when `MARKETRIG_EXPERIMENT_HITHINK_API_KEY` is unset. Before it asks you for anything, the harness has already saved your key the way the Settings block does — `PUT /research/hithink`, one real validating request, so a wrong key fails the cell there — created the desk on the cell's runtime, and read `600519.XSHG` from HiThink itself, checking that observation's `provider: "hithink"`, its `calendar` (`HITHINK` or `WEEKDAY`), and its null `source_time_ns`.

**One sitting, on two staged trading days, at any hour** (`sdd/features/a-share-engine/SPEC.md` §6, per AE-10). T+1 locks a lot until the next trading day, so this daemon's nodes are seeded at 10:00 Asia/Shanghai on the *buy* day, and the harness moves them to 10:00 on the *sell* day once the buy has filled and the same-day sell has been refused. The sell day is the most recent weekday whose 09:30 Shanghai has passed — so the provider already holds its own bar — and the buy day is the weekday before it. Only the trading dates are staged: the calendar, each day's bar and every snapshot come from the real service on both legs. Nothing about the sitting depends on the hour you start at: MarketRig's own session clock is 10:00 on each staged day, so a sitting that runs past 14:57 on your clock is not cut off, and the seam keeps this daemon polling the real service after the close. Expect the printout's `Day:` line to name both days, and the session to see `band_date` reading the buy day.

What the hour does decide is the order type. Outside real Shanghai trading hours the snapshot is simply the day's last and does not move, so ask for **MARKET** orders on both legs: a LIMIT would wait for a later observation whose volume has grown, and after the close that never arrives. Inside trading hours either works.

The printout's `Day:` line also says whether the buy day is executable. If it reads `NOT EXECUTABLE` — a holiday the weekday step cannot see — stop the run (§11); the harness records the reason and the sitting cannot buy.

Then, as in E4 and E6, MarketRig launches the runtime and **your console becomes the desk's terminal**. Answer the first-launch questions of §6.1 and nothing more. Then:

1. **Two research reads.** Ask the session to read `600519.XSHG`'s latest annual income statement and its current valuation through the two command lines the printout spells out (`marketrig research hithink a-share/financials/income-statements …` and `… a-share/valuations/snapshot …`). The session inherits `MARKETRIG_TEST_DATA_ROOT`, so the CLI reaches this daemon on its own. Success is `code == 0` in HiThink's own envelope; the passthrough writes no row and no event, so the harness records this aspect `INCONCLUSIVE` by construction and only you can judge what came back.
2. **The band, under its disclosed assumption.** Ask the session to read `600519.XSHG` on the desk's `market/quotes` resource before it trades: `prev_close`, `limit_up`, `limit_down` and `band_date`, and beside them the `execution` object with `band_date_inferred: true`, `source_delay: UNKNOWN` and `fill_policy: HITHINK_SAMPLED`. The reference is the provider's and the date is inferred from receipt — neither is certified, and that is the assumption every order below rests on.
3. **Buy one lot.** Ask the session to buy 100 shares of `600519.XSHG` through `submit_order`, as a MARKET order: it fills against the current snapshot at once. The supported-session window is not what you are waiting on — the staged clock is inside it — and a LIMIT is what would keep you waiting, since it needs a later observation whose volume has grown. Only `600519.XSHG` counts: the harness ignores trades of any other instrument, and they will not move this protocol on — if the session buys something else, steer it back to `600519.XSHG` before step 4.
4. **Watch the T+1 refusal.** Ask it to sell that same lot *now*. It must be refused: the day's buy is locked until the next trading day, and the refusal names the locked quantity. That refusal is the point of the first half.
5. **Wait for the harness's second printout.** It moves this desk's trading day on to the sell day, which unlocks the lot, waits for the instrument to re-open on the real calendar, and prints the sell instruction on the same console (it is also in `<bundle>\instructions-e7-sell.txt`). Then ask the session to sell the lot, as a MARKET order again. The native cycle closes and its `EVALUATION` prompt queues: that is the cell complete, in this one sitting.
6. **Nothing else.** The harness then removes the key (`DELETE /research/hithink`), stops the daemon, and greps every file of the bundle for it. A research read after that answers `RESEARCH_UNCONFIGURED`: that is the cell ending, not a fault.

What fails and what does not: the key's acceptance, the provider row, the `CN` observation's three fields, the clock seam's own answer, the evaluation queued behind any cycle that did close, and the key's absence from the bundle are MarketRig's own and fail the cell. Whether the session read anything, bought, asked for the refused same-day sell, or sold on the staged day are the agent's and end `INCONCLUSIVE` with their evidence.

Three limitations this cell cannot remove; the harness writes all three into `observations.jsonl` so no reading of the bundle can forget them. The provider publishes no source timestamp, so every observation's delay is `UNKNOWN` and no fill here is a claim about market time. A holding carried across a corporate action is not adjusted, so a dividend or split date inside the hold makes the realized figure incomparable — hold across one only with that limitation explicitly recorded. And both legs of this cell trade against the same day's real quotes minutes apart under two staged trading dates, so its realized figure is a harness artifact either way: never attribute any of it to strategy.

## 9. Drive the session — E8

E8 is slice 015's scenario (`sdd/features/event-triggers/SPEC.md` §7.2): **a producer hands the desk its findings**. It runs last in the cell, on its own daemon and desk, and needs nothing but the cell's runtime — no key, no prerequisites, and no trade. Before it asks you for anything, the harness has discovered the runtime, created the desk on it, and defined one trigger through `marketrig trigger create`: `review-research`, recurring, **with no schedule**, whose brief asks for a one-sentence summary of whatever input it is handed. Nothing is due; the job exists and waits.

Then, as in E4, E6 and E7, MarketRig launches the runtime and **your console becomes the desk's terminal**. Answer the first-launch questions of §6.1 and nothing more. Then:

1. **Watch the hand-off.** The harness plays the producer: one `marketrig trigger invoke review-research --request-id <id> --input <three lines>`, with three lines of made-up findings. After MarketRig's orientation you must see `MarketRig TRIGGER_RESULT <id>:` arrive as the session's own input with those three lines inside it — this time because something asked for the job, not because a clock came round.
2. **Let it answer.** The brief asks for a one-sentence summary. Read what the session says. Whether it obeyed is the agent's and is recorded `INCONCLUSIVE`; that MarketRig delivered, to which runtime and which native session, is the daemon's own and fails the cell if its rows disagree.
3. **Watch the replay do nothing.** The harness then repeats the same request id. Nothing may reach the session a second time: the answer is `DUPLICATE` carrying the first firing, and the harness checks there is still exactly one firing and no new prompt. It prints a line saying so before it repeats — the runtime's full-screen redraw usually paints over it, so do not wait to see it.
4. **Nothing else.** No order, no memory, no skill. The cell ends when the replay is recorded.

What fails and what does not: the acceptance, the firing's request id and input, the delivery the rows must agree with, the `DUPLICATE`, and the absence of a second firing or prompt are MarketRig's own and fail the cell. Whether the three lines appeared as the session's input and what the agent made of them are yours to confirm on the console and end `INCONCLUSIVE`.
...
- E7 skipped entirely reads as one `INCONCLUSIVE` line naming `MARKETRIG_EXPERIMENT_HITHINK_API_KEY`. A cell that ran ends with a note saying the key is in no file of the bundle and a note recording its three limitations; its research-reads leg is `INCONCLUSIVE` by construction. A complete cell carries the note naming the day it staged (`from`/`to`) and the note naming the closed cycle and its queued evaluation; a buy that never happened, a same-day sell the session never asked for, a sell it never asked for on the moved-on day, or a staged day the provider's calendar refused is `INCONCLUSIVE` with what the harness did see;
- E8's agent-dependent legs are the two the console shows: whether the three lines appeared as the session's input and what it answered. A cell that ran ends with a note naming the `DUPLICATE` and the prompt states it left; a session never started, never ready, or a result never delivered is `INCONCLUSIVE` with what the harness did see, and a second firing or a second prompt after the replay fails the cell;
...
- Keep all six bundles per cell: `experiment-<cell>-<stamp>`, `experiment-e3-<cell>-<stamp>`, `experiment-e4-<cell>-<stamp>`, `experiment-e6-<cell>-<stamp>`, `experiment-e7-<cell>-<stamp>`, and `experiment-e8-<cell>-<stamp>`. The bundle is the evidence; the `.codex\config.toml` and `.mcp.json` inside it carry no secret, only the data-root path — `<bundle>\data\runtime\credentials.json` is the relocated credential store and holds a provider key and E6's OpenViking seed, and `<bundle>\data\openviking\plugin\<desk-id>\ovcli.conf` holds that desk's derived OpenViking key, both by design. E6's bundle also carries the provisioned virtual environment under `data\openviking\venv\`; delete that subdirectory if the bundle has to be kept small, and nothing else. E7's bundle carries no HiThink key anywhere — the cell removes it from that credential store before it ends and fails if the key is in any file of the bundle.
