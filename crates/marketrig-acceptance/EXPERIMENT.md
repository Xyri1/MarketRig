...
2. **Close one cycle.** Ask the session to buy one lot — 100 shares — of `600519.XSHG` through `submit_order`, and then to sell that same lot. That closes a position cycle and MarketRig queues its `EVALUATION` prompt.
3. **Nothing else.** The harness then removes the key (`DELETE /research/hithink`), stops the daemon, and greps every file of the bundle for it. A research read after that answers `RESEARCH_UNCONFIGURED`: that is the cell ending, not a fault.

What fails and what does not: the key's acceptance, the provider row, the `CN` observation's three fields, the evaluation queued behind any cycle that did close, and the key's absence from the bundle are MarketRig's own and fail the cell. Whether the session read anything, and whether it closed a cycle, are the agent's and end `INCONCLUSIVE` with their evidence.
...
- E7 skipped entirely reads as one `INCONCLUSIVE` line naming `MARKETRIG_EXPERIMENT_HITHINK_API_KEY`. A cell that ran ends with a note saying the key is in no file of the bundle; its research-reads leg is `INCONCLUSIVE` by construction, and a cycle never closed is `INCONCLUSIVE` with the prompt states the harness did see;
