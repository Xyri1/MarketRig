...
- Pick the time. Before asking you to do anything, the harness waits up to 60 seconds for one `LIVE` observation from Yahoo, so at least one catalog market must be in session (feature SPEC §2.2, Monday–Friday, no holiday calendar):
  ...
  E7 reads and trades `600519.XSHG`, so time the cell so the Shanghai session is still open when it runs — it is last, an hour or two in. Its own precheck is HiThink's rather than Yahoo's and does not need the session: the `CN` leg polls once at subscription whatever the phase, so the observation lands either way, but the round trip fills against a fresher price inside it.
...
3. **Buy one lot, inside a session.** Ask the session to buy 100 shares of `600519.XSHG` through `submit_order` while Shanghai is inside a supported session (09:30–11:30 and 13:00–14:57).
4. **Watch the T+1 refusal.** Ask it to sell that same lot *today*. It must be refused: today's buy is locked until the next trading day, and the refusal names the locked quantity. That refusal is the point of the sitting.
5. **The sitting is `PARTIAL`.** T+1 means one sitting cannot complete this cell. Rerun it on a later trading day with `MARKETRIG_ACCEPTANCE_OUT` pointing at the same bundle and ask the resumed session to sell the lot; only then does the native cycle close and its `EVALUATION` prompt queue, and only then is the cell complete. Never record the first sitting as a pass.
...
What fails and what does not: the key's acceptance, the provider row, the `CN` observation's three fields, the evaluation queued behind any cycle that did close, and the key's absence from the bundle are MarketRig's own and fail the cell. Whether the session read anything, bought, asked for the refused same-day sell, or came back on a later day to close the cycle are the agent's and end `INCONCLUSIVE` or `PARTIAL` with their evidence.

Two limitations this cell cannot remove; the harness writes both into `observations.jsonl` so no reading of the bundle can forget them. The provider publishes no source timestamp, so every observation's delay is `UNKNOWN` and no fill here is a claim about market time. And a holding carried across a corporate action is not adjusted, so a dividend or split date inside the hold makes the realized figure incomparable — hold across one only with that limitation explicitly recorded, and never attribute the difference to strategy.
...
- E7 skipped entirely reads as one `INCONCLUSIVE` line naming `MARKETRIG_EXPERIMENT_HITHINK_API_KEY`. A cell that ran ends with a note saying the key is in no file of the bundle and a note recording the unknown source delay and the unadjusted corporate action; its research-reads leg is `INCONCLUSIVE` by construction. A sitting that bought and saw the same-day sell refused ends `PARTIAL` — that is the expected shape of a first sitting, not a defect — and only a later sitting that closes the cycle reads as complete; a buy that never happened, or a same-day sell the session never asked for, is `INCONCLUSIVE` with the prompt states the harness did see;
