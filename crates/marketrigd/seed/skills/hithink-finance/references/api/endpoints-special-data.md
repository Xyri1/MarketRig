...
```bash
# 今日涨停池，按涨停时间排序
marketrig research hithink a-share/special-data/limit-up-pool --param sort_field=limit_up_time --param sort_dir=asc --param size=50

# 指定交易日
marketrig research hithink a-share/special-data/limit-up-pool --param date_ms=1718294400000 --param page=1 --param size=100
```
...
```bash
marketrig research hithink a-share/special-data/limit-up-ladder
```
...
```bash
# 全部当日异动
marketrig research hithink a-share/special-data/anomaly-analysis-list

# 仅涨停和跌停
marketrig research hithink a-share/special-data/anomaly-analysis-list --param tag_codes=LIMIT_UP,LIMIT_DOWN
```
...
```bash
# 单只股票异动原因
marketrig research hithink a-share/special-data/anomaly-analysis-stock --param thscodes=600519.SH

# 批量查询
marketrig research hithink a-share/special-data/anomaly-analysis-stock --param thscodes=600519.SH,000001.SZ,300750.SZ
```
...
```bash
marketrig research hithink a-share/special-data/skyrocket-list --param period=day
```
...
```bash
marketrig research hithink a-share/special-data/hot-stock-list --param period=hour
```
...
```bash
marketrig research hithink a-share/special-data/hot-stock-list-history --param date=2025-06-30
```
...
```bash
marketrig research hithink a-share/special-data/hot-stock-rank-trend --param thscode=600519.SH --param start_date=2025-01-01 --param end_date=2025-06-30
```
...
```bash
# 今日全部龙虎榜
marketrig research hithink a-share/special-data/dragon-tiger-list --param board_type=all

# 指定日期游资榜
marketrig research hithink a-share/special-data/dragon-tiger-list --param board_type=hot_money --param date=2025-06-30
```
...
```bash
marketrig research hithink a-share/special-data/limit-down-pool --param page=1 --param size=50 --param sort_field=last_limit_time --param sort_dir=desc
```
...
```bash
marketrig research hithink a-share/special-data/limit-break-pool --param page=1 --param size=50 --param sort_field=open_times --param sort_dir=desc
```
