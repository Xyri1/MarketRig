...
```bash
marketrig research hithink dump/market-dumps/daily-k/download-url
```
...
```bash
# Step 1: 签出 URL
DOWNLOAD_URL=$(marketrig research hithink dump/market-dumps/daily-k/download-url | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['presigned_url'])")
...
```bash
marketrig research hithink dump/market-dumps/daily-k-10d/download-url
```
...
```bash
marketrig research hithink dump/market-dumps/adjustment-factors/download-url
```
