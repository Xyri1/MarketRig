...
`ponytail:` deliberate ceilings stand with their upgrade paths in the feature decisions: one Python minor, one venv location, and pip over the bundled directory (a wider validation matrix behind the same setup route); a fixed 120 s readiness deadline and one config layout (a measured deadline when a real machine misses it); one account and one flat user per desk, with peers, workspaces, and shared `viking://agent/skills` unused; one vendored plugin commit, upgraded as a seed change the reconcile check verifies; a full rewrite of the projection on every refresh (a content-hash diff when a desk's skill set makes it measurable); and `SKILL.md` alone on the write path, auxiliary files arriving with the dict form of `data`. Three ceilings are outside MarketRig's code: the Claude plugin writes a hard-coded `~/.openviking/last_inject.md` and probe cache, which stands because the vendored plugins are unmodified; a Windows box under a deep data root needs `LongPathsEnabled` because `volcengine-python-sdk` ships wheel members past the legacy `MAX_PATH` ([SPEC §18](SPEC.md#18-implementation-deferred-contracts)); and Codex exposes no daemon-visible turn end, so the turn-end projection refresh fires only on Claude's `Stop` hook ([SPEC §18](SPEC.md#18-implementation-deferred-contracts)).
