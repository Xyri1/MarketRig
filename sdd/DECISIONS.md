...
**Decision:** The agent surface is split by what the agent is doing, and no feature appears on both halves. **MCP resources** carry market awareness — quotes, book, live positions, open orders, instrument discovery — enumerated as concrete desk-scoped resources in `resources/list`, never as URI templates, and re-read explicitly. **MCP tools** carry the money actions — order submit and cancel, two or three typed tools — whose arguments the daemon validates server-side and answers with a structured tool error, because the advertised schema is documentation, not a trust boundary. The **`marketrig` CLI** carries everything else: `history`, `desk`, `trigger`, `skill`, `prompt`, and `session hook` — durable records, file-payload actions such as trigger code and RRULEs, and skill writes. The **memory plane** is neither: it is the seeded OpenViking plugin's own MCP server, which the agent uses for its memories and MarketRig never proxies, so no capability appears on two planes (per D83). MarketRig builds no subscription and no completion path. The daemon's SQLite remains the evidence authority for every action (D32, D38); the transcript never is. The seeded `AGENTS.md` names the resource URIs and the desk configuration accounts for the runtime's approval mode.
...
**Decision:** MarketRig owns Observe, Act, and Continuity. The agent owns Orient, Decide, and Learning through its research, files, OpenViking memory, and skills.
...
**Rationale:** Trading truth and research data have different authority and failure requirements. The MVP's evidence is the persistent loop, which needs authoritative market state and paper execution but not a bundled research library, and a coding agent already researches with its own tools. Bundling OpenBB would mean one more supervised child of the same shape as the memory child's, with its own interpreter and locked dependency set (D47, D65, D83) — packaging, supervision, and lifecycle work the MVP's evidence does not need.

`ponytail:` no bundled research in MVP; add OpenBB post-MVP as one supervised child beside the OpenViking child, with its own locked environment and installation-level connector credentials in the OS store.
...
### D16 — OpenViking is the MVP subjective memory system

**Decision:** Use OpenViking for desk-scoped experiential memory in MVP. `marketrigd` owns one installation-wide local OpenViking child and maps each desk to exactly one OpenViking user under the one `marketrig` account. Capture and recall are the seeded upstream plugins' — every turn is appended to that desk's session and recalled memory is injected at session start and before each prompt — and the agent's own memory operations are those plugins' MCP tools; MarketRig derives the user from desk identity, hands the plugins that desk's key, and never accepts an agent-supplied user or account. MarketRig does not proxy OpenViking's API through `marketrig` or `marketrig-mcp`, and exposes neither its admin routes nor its root key to agents (per D83).

**Rationale:** One per-user context store holding memories and skills behind one retrieval surface matches the desired learning loop outside an LLM context window, and plugin-driven capture removes the bookkeeping step the agent otherwise had to remember. Deriving the user from desk identity preserves desk isolation and the agent-owned learning contract.

**Contract:** [SPEC memory and skills](SPEC.md#16-memory-and-skills).

### D17 — OpenViking is agent-owned cognition, not MarketRig's system of record

**Decision:** The agent's own sessions and writes are what enters OpenViking; the daemon commits nothing of its own and never mutates a memory or a skill's content. MarketRig and Nautilus state remain authoritative for objective session, trigger, action, and trading reality.
...
### D18 — Memory persists locally and uses hosted models for MVP

**Decision:** Run the memory server and its storage locally: OpenViking's workspace under MarketRig's own data root with its local file and vector backends. Its LLM and embedding models are hosted for MVP behind one installation-wide OpenAI-compatible base URL, API key, selected VLM model, and embedding model; MVP uses no reranking model, and any later reranker is hosted, never local. The embedding dimension is measured once when the provider is saved and written into the child's configuration, because OpenViking silently assumes 2048 for a model it does not know; there is no embedding-model lock, since the local vector store carries its own dimension check. Persist the base URL and selected models, keep the API key in the OS credential store, and fetch the provider's model list live whenever the model selector is opened without persisting or caching that list. A model-list fetch failure is explicit and never falls back to a stale list. Memory unavailability makes memory capabilities explicitly unavailable but does not block daemon startup, native sessions, triggers, or paper trading.

**Rationale:** This preserves local durable memory while avoiding unnecessary native model packaging and compute requirements on the two target platforms. Local file and vector backends are sufficient for the local experimental MVP; a production storage topology is outside the MVP contract.
...
**Decision:** Keep skills as a distinct agent-owned procedural-memory layer, canonical in the desk's OpenViking user beside the experience that shaped it (per D83). Borrow the Hermes skill-improvement pattern without adopting Hermes as the product foundation. Agents may create and refine skills through `marketrig skill`; MarketRig projects them into the workspace so both runtimes load them, preserves the environment, and does not govern improvement automatically — OpenViking's own skill extraction stays off.

**Rationale:** Procedures can improve across sessions without fine-tuning or adopting another agent framework as the product foundation. One owner for a procedure and its experience is what makes a lesson and the skill it changed retrievable together.

**Contract:** [SPEC memory and skills](SPEC.md#16-memory-and-skills).

### D20 — Desk bootstrap has explicit ownership boundaries

**Decision:** MarketRig/user owns enforced configuration, the user owns the `AGENTS.md` constitution, MarketRig owns designated built-in compatibility material — the `CLAUDE.md` shim, the seeded plugin trees, and the `.agents/skills/` projection — and the agent owns research, tools, and its OpenViking memory and skills. Desk creation is the recoverable provisioning state machine `CREATING -> READY | FAILED`: persist the row first, bootstrap idempotently, recover incomplete creation on startup, and retry a failure on the same identity. `CLAUDE.md` is MarketRig-owned and contains exactly `@AGENTS.md` followed by one newline. Agent-owned seed material is never rewritten after the desk first becomes ready; `.agents/skills/` is not agent-owned, being the read-only projection of the desk's OpenViking skills (per D83). The OpenViking user, NautilusTrader, and runtime resources are provisioned lazily by their owning modules.
...
**Decision:** Each desk has one canonical skill set, held in its OpenViking user and projected read-only into `.agents/skills/` by MarketRig (per D83), because both runtimes discover skills only from files. Claude Code reaches that same physical directory through `.claude/skills`: a relative symlink to `../.agents/skills` on macOS and a directory junction to the absolute workspace path on Windows. There is still exactly one canonical owner and no second synchronized tree.

**Rationale:** Changing runtime must not fork the desk's procedural identity. Neither runtime reads a skill over HTTP and OpenViking exports none to a filesystem, so one read-only projection is the piece neither side provides — and a tree the runtime cannot write is a tree the projection cannot lose.

**Contract:** [SPEC memory and skills](SPEC.md#16-memory-and-skills).

### D22 — Desk self-improvement follows a modified OODA loop

**Decision:** MarketRig's defining loop is **Observe → Orient → Decide → Act → Evaluate → Learn → Observe**. MarketRig exposes authoritative observations and actions, persists continuity, and queues an evaluation prompt for every realized-P&L event. The agent selects relevant evidence, treats realized P&L as the reward signal, decides whether anything was learned, and when useful states the lesson in the session its OpenViking plugin captures and improves reusable procedures in that desk's canonical skills. Each desk's `AGENTS.md` always carries this ownership loop; MarketRig seeds an on-demand improvement skill into the desk, after which the agent owns and may evolve it. MarketRig does not require a canonical transcript, decision-attribution model, reflection cadence, or daemon-authored memory/skill mutation.
...
### D47 — The installer bundles wheels, not an interpreter: the operator names Python 3.12 and Node

**Decision:** Package with Tauri 2 as per-user NSIS on Windows and a DMG-distributed app on Apple Silicon macOS. The bundle carries `marketrigd`, the `marketrig` CLI, and `marketrig-mcp` as native executables, plus the locked per-platform wheel set `openviking-wheels/<platform>/` and nothing else interpreted. MarketRig ships **no** portable CPython and no Node: the operator names an absolute Python 3.12 and an absolute Node ≥ 22, each validated by running it, and the daemon provisions its own private virtual environment offline from the bundled wheels (D65, D83). Nothing else in MarketRig depends on either runtime.

**Rationale:** The user chose user-installed prerequisites over bundled runtimes, and validating the named executable is the shape R3 already uses for the agent runtimes. Bundling the resolved wheel set still keeps first run offline — `--no-index` makes a missing wheel a build-time failure rather than a desk stuck mid-setup — while dropping an interpreter MarketRig would have to security-update in every release. The memory server is a Python package and its plugins are Node scripts, so both runtimes have to exist somewhere; naming them is cheaper than shipping them.
...
**Decision:** `marketrigd` alone stores provider secrets in the native per-user credential store — macOS Keychain or Windows Credential Locker — through one maintained, exactly pinned Rust binding (`keyring` is the candidate — verified on crates.io 2026-09-01, 4.x line; selection confirmed at plan time). SQLite keeps only non-secret configuration and opaque references; native-store failure has no plaintext fallback. Secrets never appear in SQLite, logs, prompts, URLs, or CLI output. There are exactly two stated exceptions, both the daemon's own files and children and neither on a runtime process environment (per D83): the supervised OpenViking child receives the provider key and the daemon's per-start root key through its own process environment, which its configuration loader expands from `${…}` references so no key is in the file; and each desk's OpenViking key is written plain into that desk's daemon-owned `ovcli.conf`, mode `0600` under the data root, because the vendored plugins read their credential from that file and the Codex app-server hands them no per-desk environment.
...
### D65 — Memory is one supervised `openviking-server` child in a MarketRig-provisioned venv, with one OpenViking user per desk and one hosted endpoint

**Decision:** MarketRig installs `openviking`, pinned exactly and verified to install offline on both MVP platforms, into its own virtual environment under the data root from the bundled wheel set (D47), and `marketrigd` runs it as one supervised loopback child — `openviking-server` on a daemon-picked port, in `api_key` mode behind a per-start root key, started with the daemon, ready on `/ready`, `UNAVAILABLE` on its first loss until Retry, and never automatically restarted. Its workspace, configuration, and redirected home live under `<data root>/openviking/`. Each desk is one OpenViking user `desk-<hex>` under the one `marketrig` account, its key derived from one credential-store seed and obtained at every readiness; agents never name an account or a user. One OpenAI-compatible base URL and key serve both the VLM and embeddings, with the embedding dimension measured once at provider save and no reranker configured. Capture, recall, and the agent's memory tools are the seeded upstream plugins'; skill writes are `marketrig skill put|delete`, forwarded by the daemon under the desk key; MarketRig keeps no content store of its own. The mechanics are D83's.

**Rationale:** OpenViking is configured by one file plus environment expansion and can fail independently of trading, so a supervised child with an explicit unavailable state delivers the degradation and isolation promises with the least new machinery. A per-user store rather than per-desk databases is OpenViking's own isolation boundary, and `api_key` mode is what makes a user key reach only that user's data. The server being a Python package and its plugins Node scripts is the whole reason MarketRig names two prerequisites at all (D47), and this child is the one place that language boundary lives.
...
**Decision:** MarketRig is accepted by one ordered scenario chain in its own acceptance harness (D75) that drives the real `marketrigd`, the real NautilusTrader sandbox on live public market data, a real OpenViking child behind a real hosted endpoint, and the real Codex CLI or Claude Code through public surfaces only — `marketrig --json`, the loopback API, the desk's MCP surface, the per-desk terminal socket, workspace files, and read-only SQLite. There is no test-only product surface. In the attended mode every native session start is operator-attended through a terminal relay so runtime dialogs are answered by a person. Agent-owned steps are instructed by a user-owned `AGENTS.md` addendum and verified by side effects — a captured session under the trading desk's own OpenViking user and none under the other's, a skill naming the cycle, a later session reusing both — with bounded attempts on fresh trade cycles; mechanical assertions get none. The chain includes one clean restart inside a trigger's lead window and one hard kill with a live session, runs in a relocated root with run-stamped desk names, and must pass in every platform-and-runtime cell with an evidence bundle each.
...
**Decision:** The acceptance harness runs the same ordered scenario chain in two modes. The **gate** is unattended and deterministic: the daemon, the CLI, the database, the NautilusTrader sandbox, and every public surface are real, and the sandbox is driven by the milestone's deterministic stand-in quote source where one exists (D76) and by live public data where it does not, while the runtime CLIs and the memory child are the harness's stand-ins — a stub Codex app-server with a scripted agent that performs the addendum's steps on the prompts it receives, the stub Claude probes, and a stand-in OpenViking server — so the chain proves MarketRig's mechanics with no model, no key, no first-run dialog, and no operator. The **experiment** is the operator-attended run of the same chain on the real Codex CLI or Claude Code and a real OpenViking child behind a real hosted endpoint, once per release candidate per cell, whose bundle is archived as the evidence that a real agent closes the loop. Both modes are Cargo test targets in MarketRig's own workspace (D53, D60); their exact invocations are set at plan time.
...
**Decision:** `installation_settings` gains `trigger_code_policy` (default `REQUIRE_APPROVAL`), `paper_order_policy` (default `ALWAYS_ALLOW`), and `delivery_mode` (`QUEUE`, with a `CHECK` that admits no other value), each read by plain SQL inside the unit that needs it, with no cache and no per-desk override. `GET`/`PUT /settings/policies` is their one resource: it reports `steer_available: false`, refuses `STEER` as `STEER_DISABLED`, and appends one installation-wide `POLICY_CHANGED` per field that actually changes. There is no approvals table — an approval **is** the `code_snapshots` row or the `trading_actions` row. Both carry the one vocabulary: `approval` is `ALWAYS_ALLOW | PENDING | APPROVED | DENIED` with `decided_at_ns` null exactly while `PENDING`, and `triggers.next_occurrence_ns` is non-null only when the trigger is enabled, undeleted, and its snapshot is null, `ALWAYS_ALLOW`, or `APPROVED`, so the scheduler's one query needs no join, an unapproved trigger is never due, enable and disable cannot resurrect it, and an approved elapsed one-off stays null. An order under **Require approval** commits `approval = PENDING` with no outcome, enqueues nothing, answers a null order projection, and returns that same row for a repeated idempotency key; approval requires a `RUNNING` node hosting the desk's book and then re-enters the ordinary accept path exactly where acceptance left it, denial is terminal with the `DENIED` failure code as the stored outcome, and a `CANCEL` is never gated. Approvals and policies are reachable only through REST and the desktop: no `marketrig` command reads or writes a policy, lists approvals, or decides one, and the agent sees the state through the records it already reads. A decision queues no daemon prompt.
...
**Decision:** The Codex app-server and the OpenViking child are recorded in `runtime/children.json` when the daemon launches them and removed when it stops them; crash recovery's first step terminates any recorded child of a prior daemon whose pid is alive and whose command line still carries the recorded arguments (a shim may replace the executable path), drops every record either way, and reports each outcome in the `RECOVERY` event. Windows discards records only, since its Job Object already ends the children. Nothing is recorded for trigger scripts.
...
**Decision:** The first trading milestone is **equity paper trading across three markets — US, Hong Kong, and China A-share — on one keyless Yahoo Finance `DataClient`** (D39) against the NautilusTrader sandbox, with one multi-currency paper account per desk (USD, HKD, and CNY balances; realized P&L in each instrument's own currency). Full crypto is not dropped or narrowed: the Kraken crypto environment (per D74) lands in a later milestone. The client stays thin: transport rides the workspace's one reqwest line (0.13.4, verified 2026-09-01 — the same line `nautilus-network` pins, so one HTTP stack serves both), and the community `yahoo_finance_api` crate — which owns the crumb/cookie and user-agent quirks, at bus factor one — was the candidate endpoint layer, pinned or replaced at plan time; it was **replaced** at plan time (slice 002, source verified 2026-09-01) because its 5.0.0 release exposes no chart-URL override, which the acceptance seam requires, so the endpoint layer is MarketRig's own chart-URL construction and response parsing. The alternatives were evaluated and rejected on 2026-09-01: Databento on cost and signup terms; Alpaca — the only free source with a real bid/ask, terms verified clean — on market coverage, since it serves US listings only; the remaining keyed vendors on licence clauses (display-only use, or no durable storage). Four mitigations for the unofficial-API risk bind before implementation:
...
### D78 — The equity milestone settles the market plane: evidenced feed freshness, a curated catalog, a node-judged cash book, verbatim history with one snapshot, five resources and two tools, and a stand-in feed

**Decision:** Milestone R1 settles the market plane's mechanics; the full record is [`features/r1-equity-paper-trading/`](features/r1-equity-paper-trading/SPEC.md) (R1-1…R1-9). **Feed:** one polled Yahoo chart client per node on MarketRig's own thin endpoint layer — once at subscription whatever the phase, then only while the market is `OPEN`, every 30 seconds and every 10 seconds where the desk has exposure; HTTP 429 retried 8 times 400 ms apart; exhaustion leaves the last observation standing as `DEGRADED`, never a silent substitution. The feed declares no delay figure — `exchangeDataDelayedBy` verified null on all three exchanges on 2026-09-01 — so freshness is evidenced only by source time against receipt time, both on every read. **Calendars:** static weekly sessions in each market's IANA zone (US 09:30–16:00; Hong Kong 09:30–12:00 and 13:00–16:00; China A-share 09:30–11:30 and 13:00–15:00), yielding one fact, the phase `OPEN | CLOSED`, which gates polling and labels observations and never gates an order; no holiday calendar. **Catalog:** tradable instruments are curated product data compiled into the daemon — Nautilus identifier, feed symbol, market key, currency, fixed tick, lot — fifteen at R1; anything else answers `INSTRUMENT_UNKNOWN`, and extending it is a data change. **Book:** one NautilusTrader cash account with netting positions per desk, seeded once with 100,000 USD, 1,000,000 HKD, and 1,000,000 CNY and never converting between them, realized as one venue account per venue; `MARKET` and `LIMIT` orders, GTC, plus cancel; the daemon validates form (`ORDER_INVALID`) and the node alone judges sufficiency, every refusal answering `ORDER_REJECTED` with the node's reason; fees at each market's declared per-side rate — US 0 bp, Hong Kong 11 bp, China A-share 3 bp — through an explicitly configured maker-taker model reading the rates each catalog instrument carries; 64-bit precision declared explicitly and asserted at node start. **Identity:** every mutating command carries a caller-supplied `action_id` (`[a-z0-9-]{1,64}`, unique per desk), recorded in `trading_actions` before the sandbox sees the order; a repeat returns the stored record and acts on nothing, refused actions included; the submit's `action_id` is the order's `client_order_id`. **History:** migration 2 — `trading_actions`, `order_events`, `fills`, `position_events`, `position_cycles`, `prompts`, `book_snapshots` — every row desk-scoped, every sandbox payload verbatim as versioned JSON beside the normalized columns, listings as query projections, a through-zero fill inserting the cycle and its `QUEUED` `EVALUATION` prompt in one unit, and exactly one restoration snapshot per desk rewritten in the same unit as every book-changing event; MarketRig holds no position objects outside the node. **Nodes:** a desk's node starts lazily on its first market-plane operation, restores from its snapshot before it is published — resting limit orders re-placed under their original client order identifiers — and appends `TRADING_NODE_STARTED`; a start failure appends `TRADING_NODE_FAILED`, answers `MARKET_UNAVAILABLE` for that desk only, and may be retried by the next operation. **Surface:** `marketrig-mcp --desk <name-or-id>` binds one desk, discovers the daemon as the CLI does, enumerates five aggregate resources (`quotes`, `book`, `positions`, `orders`, `instruments` under `marketrig://desk/<name>/`) with a zero cache lifetime, and exposes `submit_order` and `cancel_order`, the daemon validating everything but one path-segment guard; the CLI gains `history <orders|fills|cycles>` over durable records only; new codes `INSTRUMENT_UNKNOWN`, `ORDER_NOT_FOUND`, `ORDER_INVALID`, `ORDER_REJECTED`, `DESK_NOT_READY`, and `MARKET_UNAVAILABLE`. **Acceptance:** a third seam variable, `MARKETRIG_TEST_QUOTE_URL`, honored only alongside the data root and outranking `MARKETRIG_TEST_NO_TRADING`, points the feed at a harness-owned server speaking the chart endpoint's shape with scripted prices, under which the poller treats every phase as `OPEN`; the gate's G12–G20 run on it, and real Yahoo rides only the attended cells, whose first content is E1 and E2 — a real Codex CLI session and a real Claude Code session reading the quote resource and submitting and cancelling a paper order, registration by hand until the runtime adapters take it over. Pins: `nautilus-*` `=0.62.0` with default features off on every entry, `reqwest =0.13.4`, `rust_decimal =1.42.1`, `chrono =0.4.45`, `chrono-tz =0.10.4`, `rmcp =3.2.0`, `anyhow =1.0.104`, `async-trait =0.1.92`; the `yahoo_finance_api` candidate D76 named was replaced by MarketRig's own chart client because its 5.0.0 release exposes no URL override.

**Rationale:** Each clause is the smallest honest mechanism. A delay constant nobody can observe would be an assumption dressed as a contract, so timestamps carry freshness. Two-tier polling only during sessions keeps the steady state near half a request per second, the cheapest 429 hygiene there is. A curated catalog is the smallest correct place for metadata the feed does not declare and the sandbox silently requires. A cash account makes the physics forbid shorting and leverage, and letting the node alone judge sufficiency keeps the daemon from computing a trading fact (D38). Caller-supplied identity is the only identity that can tell one intent retried from two. Verbatim events plus one rewritten snapshot is D38's capture rule made literal and the cheapest thing restoration can read. Aggregate resources keep the eager lister's cost at five whatever the catalog grows. Faking the provider at the HTTP boundary exercises every line of the real client. Every clause was carried through the module checks, the gate on both platforms, and four attended cells on 2026-09-02.

`ponytail:` four deliberate ceilings stand, each with its upgrade path recorded in the feature decisions: each desk's node polls independently (an installation-wide fetch layer when desk count makes the duplication matter); a static catalog with fixed ticks (a maintained source or per-band tick logic behind the same seam); no holiday calendar (a per-market holiday table if evaluation evidence shows the mislabel misleads the agent); and a symmetric per-side fee schedule folding statutory round-trip charges (a side-aware fee model behind the same explicit-configuration seam).

**Contract:** [SPEC §12.2](SPEC.md#122-shared-reads-and-isolated-books), [SPEC §12.3](SPEC.md#123-trading-actions-and-approvals), [SPEC §12.4](SPEC.md#124-ledger-and-provenance), [SPEC §13.1](SPEC.md#131-mcp-trading-plane), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), and [SPEC §17](SPEC.md#17-verification).

## Scheduled triggers

### D79 — The scheduling milestone settles later work: wall-clock recurrence through the zone, one acceptance unit, fingerprinted snapshots under one containment primitive, FIFO executions resolved by recovery, and reference prompts

**Decision:** Milestone R2 settles scheduled triggers and trigger-code execution; the full record is [`features/r2-scheduled-triggers/`](features/r2-scheduled-triggers/SPEC.md) (R2-1…R2-8). **Schedules:** a one-off is an absolute instant (`at`, stored `at_ns`); a recurring schedule is one RRULE value, a naive local `dtstart`, and an IANA `tz`, iterated by `rrule =0.14.0` in a fixed UTC frame over the wall clock and resolved candidate by candidate through `chrono-tz` — a gap skipped, an overlap taking the earlier instant — so the DST words of D37 are the literal code path; `FREQ=SECONDLY`, `COUNT`, `UNTIL`, `BYSECOND`, prefixed or multi-line rule text, unknown zones, and a one-off in the past answer `TRIGGER_INVALID`; the projection is always the first candidate after a reference (the accepted occurrence after acceptance; now after a miss, enable, reschedule, or creation), recomputed from the anchor. **Scheduler:** one task waits on the earliest projection, a 60-second recheck, or a `Notify` every mutation signals, then runs one `BEGIN IMMEDIATE` unit over every due trigger: at or after this daemon's start and no more than 60 seconds late → a firing (occurrence, brief, context, revision, snapshot id) plus, for code-free firings, its `TRIGGER_RESULT` prompt, and the one-off consumed or the rule advanced; otherwise one `TRIGGER_MISSED` event summarizing the range with its candidate count (capped at 10,000), the one-off terminal, the rule advanced past now. The unique firing index is the duplicate guard. **Snapshots and execution:** `source` ≤ 256 KiB, a suffix, `argv` with exactly one whole-argument `{script}`, `timeout_secs` 1–3,600 default 300, SHA-256-fingerprinted with `ring`; `approved_at_ns = created_at_ns` under the fixed Always allow; the source written under `runtime/scripts/`, spawned without a shell in the desk workspace with the four `MARKETRIG_*` identifiers and the version-1 firing document on standard input, streams captured under 1 MiB and 256 KiB caps and stored as bytes on the execution row; outcomes `EXITED`, `TIMED_OUT`, `OUTPUT_LIMIT`, `SPAWN_FAILED`, `QUIT`, `DAEMON_LOST`, each with its prompt in one unit. Containment is `process-wrap =9.1.0` — `ProcessSession` on macOS, `JobObject` with `KillOnDrop` on Windows — the one spawn-and-terminate path for every managed child from here on. **Executor:** one task claims, per desk with no `RUNNING` execution, the oldest unclaimed code-bearing firing as an `executions` row under this daemon's UUID and runs it; desks run concurrently, a desk one at a time; shutdown terminates every group and persists `QUIT` inside the 5-second bound; recovery completes another daemon's `RUNNING` rows as `DAEMON_LOST` with their prompts and lists them under `executions_lost`, while never-claimed firings stay pending. **Prompts:** `TRIGGER_RESULT` is born `QUEUED` with a reference payload (trigger, firing, brief, context, execution summary); the streams stay on the row. **Attribution:** the shared daemon client sends `X-MarketRig-Trigger-Id` and `X-MarketRig-Firing-Id` from `MARKETRIG_TRIGGER_ID` and `MARKETRIG_FIRING_ID`; the order routes validate them against the firing row into `source TRIGGER` on the action, else `ATTRIBUTION_INVALID`. **Surface:** migration 3 (`code_snapshots`, `triggers`, `firings`, `executions`; `prompts`, `trading_actions`, `operational_events` rebuilt); triggers, firings, and prompts as REST resources under the desk with soft deletion and per-desk live-name uniqueness; the CLI's `trigger` and `prompt` groups; new codes `TRIGGER_INVALID`, `TRIGGER_NAME_TAKEN`, `TRIGGER_NOT_FOUND`, `FIRING_NOT_FOUND`, `PROMPT_NOT_FOUND`, `ATTRIBUTION_INVALID`. **Acceptance:** the harness's `trigger-code` binary drives G21–G26 on both platforms, and E3 attends a real session writing its own trigger code. Pins: `rrule =0.14.0` (default features off), `process-wrap =9.1.0` (`tokio1`, `process-session`, `job-object`, `kill-on-drop`), `ring =0.17.14`, tokio's `process` feature.

**Rationale:** Each clause is the smallest mechanism that makes a root sentence literal. Resolving each wall-clock candidate through the zone crate is the only path that makes D37's DST words the code rather than a property of the recurrence crate. One task and one unit are the whole of D40's "durable projection plus in-memory wake"; the daemon-start rule makes downtime misses fall out of the same lateness comparison, so startup needs no reconciliation pass. The claim row is the at-most-once fact D35 demands, and keeping it in SQLite lets recovery settle it without knowing anything about the process that is gone. A reference payload keeps a megabyte of output from being stored twice. Two headers in the one client both binaries share is attribution with no new surface. A harness-built Rust binary is the only trigger executable guaranteed on every CI runner. Every clause was carried through the module checks, the gate on both platforms, and the attended cells.

`ponytail:` three deliberate ceilings stand with their upgrade paths in the feature decisions: every projection iterates from the anchor rather than a stored cursor (a persisted cursor if minutely rules with old anchors show in the scheduler's timing); a child spawning descendants between `CreateProcess` and job assignment escapes the Windows job (a suspended-start spawn through the same wrapper); and approval is fixed at Always allow (R5 adds the policy and its surfaces, with the snapshot's `approved_at_ns` already the gate).

**Contract:** [SPEC §8](SPEC.md#8-trigger-model), [SPEC §9](SPEC.md#9-trigger-code-execution), [SPEC §10](SPEC.md#10-scheduled-trigger-semantics), [SPEC §11.1](SPEC.md#111-states-and-ordering), [SPEC §12.3](SPEC.md#123-trading-actions-and-approvals), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), and [SPEC §17](SPEC.md#17-verification).

## Runtime adapters and delivery

### D80 — The delivery milestone settles how the harness reaches a real agent: validated launch targets, one PTY manager, a Codex app-server and a Claude Channel bridge behind one dispatcher, resume-first activation as prompt rows, and a stand-in runtime in the gate

**Decision:** Milestone R3 settles runtime discovery, the terminal manager, both runtime adapters, activation, delivery, session controls, and their durable evidence; the full record is [`features/r3-runtime-delivery/`](features/r3-runtime-delivery/SPEC.md) (R3-1…R3-9). **Discovery:** `runtimes` holds one row per runtime (`codex`, `claude`) with `state IN ('UNDISCOVERED','AVAILABLE','UNAVAILABLE')`, the exact executable, its version, and a failure code; discovery runs at daemon start (step 6a, skipped under the test seam) and on `POST /runtimes/{runtime}/discover` with an optional explicit executable, resolving a bare name through the user's real environment — the login shell's `PATH` captured once per start on macOS, the user and machine `PATH` read from the registry on Windows with `.cmd`/`.bat` launched through `ComSpec` — and validating with `--version` (first dotted version; floors Codex `0.152.1`, Claude Code `2.1.258`) and `--help` markers (`app-server`; `--dangerously-load-development-channels` and `--settings`, probed differentially when hidden); failures are `NOT_FOUND`, `VERSION_UNSUPPORTED`, `CAPABILITY_MISSING` (including a path inside the Codex or ChatGPT desktop bundle, per D3), `PROBE_FAILED`. **Terminal:** one private manager over `portable-pty =0.9.0` (its Unix spawn is fork-then-exec with only `setsid` and `TIOCSCTTY`, so no spawn helper); one blocking reader per terminal into a 256 KiB reconnect ring, a bounded writer, coalesced resize, shutdown draining ≤ 2 s before terminating the tree (`killpg` on the child's session; a kill-on-close Job Object on Windows); one attachment route `GET /desks/{desk_id}/terminal` with binary bytes both ways, one text resize frame, close `4001` on supersede or a 1 MiB send-buffer overflow, and an `exited` frame; the manager answers ConPTY's `ESC[6n` with `ESC[1;1R` itself when nobody is attached, the one byte the daemon ever synthesises on a terminal (R3-9). **Codex:** one lazily started `codex app-server --listen ws://127.0.0.1:<port> --ws-auth capability-token --ws-token-file <runtime/codex-ws-token>` under the R2 containment primitive, recorded in `children.json`, reached by one bearer-authenticated JSON-RPC connection that reads broadcasts only; a session is `codex --remote … --remote-auth-token-env MARKETRIG_CODEX_WS_TOKEN -C <workspace>` or `codex resume <thread> …`, registered for `marketrig-mcp` by a MarketRig-written `<workspace>/.codex/config.toml` because `-c` overrides never reach the remote thread; the pointer is the first non-`ephemeral` `thread/started` whose `cwd` is the workspace, readiness the first time that thread is `idle` from either broadcast, delivery `turn/start` behind the idle gate plus `thread/turns/list`, interrupt `turn/interrupt`; app-server loss ends every Codex process `CONTROL_PLANE_LOST`, keeps pointers, restarts once, then marks the runtime `UNAVAILABLE` until Retry. **Claude Code:** `claude --session-id <uuid>` or `--resume <uuid>` with `--mcp-config` and `--settings` from `runtime/launch/<desk-id>/` (0600, deleted at close) and `--dangerously-load-development-channels server:marketrig-channel`; `mcp.json` registers `marketrig` and `marketrig-channel` (`marketrig-mcp --channel`, the bridge as one mode of the existing binary); `settings.json` names `SessionStart`, `Notification`, and `Stop` hooks that run `marketrig session hook`, which forwards standard input and always exits `0`; the bridge opens `GET /desks/{desk_id}/channel` only after `notifications/initialized`, that connection is readiness, each frame `{content, prompt_id, kind}` becomes one `notifications/claude/channel`, a completed write is `DELIVERED` (per D36), and Interrupt answers `INTERRUPT_UNSUPPORTED` (per D69). The launch environment is cleared and rebuilt — `PATH`, `HOME`, `TERM`, the locale variables, the Windows system set, `MARKETRIG_DESK_ID`, the Codex token — because an inherited `CLAUDE_CODE_CHILD_SESSION` silently breaks every later resume. **Dispatcher and activation:** one task woken by every prompt insert and adapter event and a 30-second recheck delivers one prompt per desk per pass, oldest first; no live session → activate resume-first, inserting `ORIENTATION` (and `DISCLOSURE` when undisclosed failures exist) as ordinary prompt rows ahead of the queue; readiness deadline 120 s, after which every `QUEUED` prompt of that desk fails `ACTIVATION_FAILED`; a dispatcher resume that exits before readiness starts new once with `SESSION_POINTER_CHANGED` (`unresumable`), explicit Continue never. States `QUEUED | DELIVERED | FAILED`; failure codes `DELIVERY_REFUSED`, `HANDOFF_UNKNOWN`, `CHANNEL_UNAVAILABLE`, `ACTIVATION_FAILED`, `RUNTIME_UNAVAILABLE`; the attempt (`attempted_at_ns`, runtime, native session) is written before the adapter call so a daemon lost mid-handoff recovers as `HANDOFF_UNKNOWN`, and a closed gate clears it again. **Session controls:** `GET /desks/{d}/session`, `POST …/session/activate {CONTINUE|NEW}`, `/interrupt`, `/exit`, `/switch {runtime}`, plus `GET /runtimes` and the discover and retry routes; Switch validates before touching anything, ends the live process `INTERRUPTED`, moves `desks.selected_runtime`, appends `RUNTIME_SWITCHED`, and does not activate; every process closes with `exit_reason IN ('EXITED','INTERRUPTED','QUIT','CONTROL_PLANE_LOST','DAEMON_LOST')` and one `SESSION_EXITED`. **Schema:** migration 4 — `desks.selected_runtime`, `runtimes`, `native_sessions (desk_id, runtime)`, `agent_processes` with a partial unique live index, `prompts` rebuilt with the three states, the two new kinds, and the attempt columns, `operational_events` widened with the fourteen R3 kinds; the `sessions` recovery step closes another daemon's open processes `DAEMON_LOST` and fails attempted-unresolved prompts `HANDOFF_UNKNOWN`, listing `sessions_lost` and `prompts_unknown`. **Acceptance:** `runtime-standin`, the harness's second helper, is registered as both runtimes by explicit path and scripted through `MARKETRIG_STANDIN_SCRIPT` on the daemon's environment, speaking exactly the consumed protocol subset — an app-server and remote TUI as Codex, a hook-running, channel-echoing session as Claude — so G27–G32 prove the adapters unattended on both platforms, and E4 attends a real firing waking a real session with the operator's console as the desk's terminal. Pins: `portable-pty =0.9.0`, axum's `ws` feature with `tokio-tungstenite =0.29.0` (`connect` only), `futures-util =0.3.34`, and the already-present `libc` and `windows` crates for tree termination.

**Rationale:** Each clause makes a root sentence literal on the real CLIs, and each was carried through the module checks, the gate on both platforms, and the attended cells. Version floors are the honest capability probe because the flags and subcommands the adapters need exist at those lines and nowhere earlier. The idle gate is the adapter-private safe-delivery gating D36 names, because `turn/start` joins an active turn rather than queueing behind it; Claude Code queues channel events itself, so no gate is needed there. Orientation and disclosure as rows keep one delivery path, one evidence shape, and one FIFO. Writing the attempt before the call is what makes completion-unknown a recoverable fact rather than a guess. A stand-in at the protocol boundary is the same move as the stand-in feed and `trigger-code`, and it turns the adapters' wire contract into executable evidence. Three of the mechanisms were forced by observation rather than design: `-c` registration not reaching the remote thread and a fresh thread having no rollout to resume (spike S), Claude Code dropping a channel notification pushed before `initialized`, and ConPTY holding a child's startup on a cursor query nobody attached could answer.

`ponytail:` deliberate ceilings stand with their upgrade paths in the feature decisions: version parsing is the whole capability check (a real protocol probe if a line keeps the flags but changes their meaning); one reader thread per terminal (an async reactor if hundreds of desks ever exist); `portable-pty` drags a second `nix` and `smol` into the graph for one PTY pair (a ~200-line in-tree PTY over `libc` and `windows`); the development-channel flag's security confirmation is answered by the operator in the attached terminal (`--channels` the day Claude Code admits a non-plugin channel); and the 4-byte cursor query is assumed to arrive in one read (a carry buffer if a split is ever seen, and no reply at all once `portable-pty` lets callers omit `PSEUDOCONSOLE_INHERIT_CURSOR`).

**Contract:** [SPEC §4.1](SPEC.md#41-installation), [SPEC §4.4](SPEC.md#44-configuration-scopes), [SPEC §4.6](SPEC.md#46-daemon-startup-and-shutdown), [SPEC §6](SPEC.md#6-managed-runtime-lifecycle), [SPEC §7](SPEC.md#7-activation), [SPEC §11.1](SPEC.md#111-states-and-ordering), [SPEC §13.1](SPEC.md#131-mcp-trading-plane), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), and [SPEC §17](SPEC.md#17-verification).

## Memory, skills, and the closed loop

### D81 — The memory milestone closes the loop: a lazily started Hindsight child under a redirected `HOME`, one credential-store provider row, derived desk banks behind four pass-throughs, seeded constitution and skill with one skills link, and a stand-in memory child in the gate

**Decision:** Superseded for memory and skills by D83 (2026-09-08); the seeded constitution, skills link, and provider row remain. Milestone R4 settles the memory child, provider settings, desk banks and the memory operations, the seeded constitution and skill, their durable evidence, and their acceptance; the full record is [`features/r4-memory-skills-loop/`](features/r4-memory-skills-loop/SPEC.md) (R4-1…R4-6). **Child:** `memory_child` is one installation row — `state IN ('UNCONFIGURED','AVAILABLE','UNAVAILABLE')`, the exact launcher path, `validated_at_ns`, a failure code and message — naming Hindsight's own `hindsight-api` console script from an environment carrying the pinned wheel; `POST /memory/discover {executable}` validates an absolute path by running `<executable> --help` within 10 s and requiring `HINDSIGHT_API_PORT` in its standard output (`NOT_FOUND`, `CAPABILITY_MISSING`, `PROBE_FAILED`), startup step 6b re-validates an `AVAILABLE` row and is skipped under the test seam like 6a, and `POST /memory/retry` clears a `CHILD_FAILED`. The child is started **lazily**, by the first memory operation that finds none live and never by startup or status: a port bound and released, a 32-byte hex bearer minted per start and held in memory only, the launcher spawned under the R2 containment primitive and recorded in `children.json`, its `HOME` (and `USERPROFILE`, `LOCALAPPDATA`) redirected to `<data root>/hindsight/` so pg0's PostgreSQL installation and the MarketRig-named instance live inside MarketRig's own root, MCP and telemetry off, one worker, and **both** its output streams captured into one 4 KiB tail, because the launcher logs to standard output while a startup traceback goes to standard error. Readiness is the first `GET /health 200`, polled every 500 ms inside 120 s (measured 12.6 s cold, 6.1 s warm), and appends `MEMORY_STARTED`; an exit or a missed deadline appends `MEMORY_LOST`, the next operation starts it once more, and a second loss with no readiness between sets the row `UNAVAILABLE CHILD_FAILED` with the child's last output line until Retry. A key the endpoint refuses is never a loss — the child may start healthy and the operation carries the failure. Every stop — Quit, a provider change, a loss whose child still lives — is `SIGTERM`, up to 3 s, then the group kill, followed by a `SIGTERM` to the postmaster `<data root>/hindsight/.pg0/instances/marketrig/data/postmaster.pid` names, because pg0 runs PostgreSQL in its own session outside the containment group; Quit stops the child after the terminals and the Codex app-server inside the existing 5 s bound. **Provider:** `memory_provider` is one row (`base_url`, `llm_model`, `embedding_model`, an opaque `key_ref`, `embedding_locked_at_ns`) written by `PUT /memory/provider`; the key goes to the OS credential store through `keyring-core =1.0.0` with the platform store crate named explicitly (`apple-native-keyring-store =1.0.2`, `windows-native-keyring-store =1.1.0`, set as the default store at daemon start) under service `marketrig` and account `hindsight-provider`, a store failure answering `CREDENTIAL_STORE_UNAVAILABLE` with nothing written; `GET /memory` answers `api_key_present` and never a key, `GET /memory/provider/models` fetches `<base_url>/models` live and answers `PROVIDER_UNREACHABLE` rather than a cached list, the first successful retain stamps `embedding_locked_at_ns` and a later different embedding model is `409 EMBEDDING_MODEL_LOCKED`, and a provider change stops a live child. Under `MARKETRIG_TEST_DATA_ROOT` the store is a 0600 `runtime/credentials.json` in the relocated root — the harness seam, never a fallback on native-store failure. **Operations:** a desk's bank is `desk-` plus its UUID without hyphens, computed per request, created by Hindsight on first use, and never named outside the daemon; `GET /desks/{d}/memory` and `POST …/memory/{retain|recall|reflect}` are synchronous pass-throughs carrying the bearer, the agent's `content`, `context`, `tags`, `query`, and `budget` verbatim, and `metadata {source, desk_id, trigger_id?, firing_id?}` from R2's attribution headers; MarketRig stores no content and appends only `MEMORY_RETAINED` and `MEMORY_RECALLED`. Limits are `content` ≤ 64 KiB, `context` ≤ 4 KiB, `query` ≤ 8 KiB, ≤ 16 tags of ≤ 64 characters, `budget IN (low, mid, high)`; timeouts 180 s for retain and reflect, 60 s for recall; codes `MEMORY_UNCONFIGURED`, `MEMORY_UNAVAILABLE`, `MEMORY_REJECTED`, `MEMORY_TIMEOUT`, `MEMORY_ERROR`. Every message the daemon lifts from the child is **redacted** first — the stored key, and any delimited token beginning with its first eight characters, becomes `<redacted>` — because Hindsight's `detail` quotes the provider, which quotes the key back. The CLI group is `marketrig [--json] memory <status|retain|recall|reflect> <desk> …`. **Seeds:** creation writes `AGENTS.md`, the `CLAUDE.md` shim, `.agents/skills/desk-improvement/SKILL.md`, and the `.claude/skills` link (a relative symlink on macOS, a directory junction through the Win32 reparse-point API on Windows) in that order, each step skipped when its target exists; after `READY` only the shim and the link are reconciled, an ordinary directory at `.claude/skills` is left alone and named in the derived workspace-status reason, and a desk created before R4 keeps its own constitution. The two seeded texts are files under `crates/marketrigd/seed/` with the desk name substituted, so their wording is a file change the checks compare byte for byte. **Schema:** migration 5 adds the two one-row tables and widens `operational_events.kind` with `MEMORY_CONFIGURED`, `MEMORY_STARTED`, `MEMORY_LOST`, `MEMORY_UNAVAILABLE`, `MEMORY_RETAINED`, and `MEMORY_RECALLED`; the child's live state and pid stay in memory and recovery needs no new step, because the child is a `children.json` record the first step already reaps. **Acceptance:** `memory-standin`, the harness's third helper, is registered as the memory launcher by explicit path, answers the probe, serves the consumed Hindsight subset behind the bearer with an in-memory per-bank store, and as `--models` is the gate's provider stand-in; G33–G37 extend the chain after G32 — configuration and secrets, two desks and one lesson, the loop closing with the `EVALUATION` prompt read as the stand-in session's own `INPUT` line and the harness then performing the agent's retain and skill edit through public surfaces, the later session's recall through both skill paths, and Hindsight lost, restarted once, then `UNAVAILABLE` while trading and triggers continue — and E5 attends the same round trip per cell on real Hindsight behind a real hosted endpoint. Pins: `keyring-core =1.0.0`, `apple-native-keyring-store =1.0.2`, `windows-native-keyring-store =1.1.0`, and `hindsight-api-slim[embedded-db]==0.9.2` as the wheel every launcher carries.

**Rationale:** Each clause makes a root sentence literal without adding a product entity. Lazy start is what keeps D18's "never blocks daemon startup" true, since a pg0 cluster and a model verification take tens of seconds; the `--help` probe is the only capability check that runs with no database and no key. Redirecting `HOME` is the one handle on where the embedded database lands, so a data root stays self-contained and two roots can run at once — and it is also why the daemon can name the postmaster it must reap. A derived bank plus no content store is D16 and D17 made mechanical: the agent chooses what is worth keeping, and nothing about a lesson exists in MarketRig but the event that one landed. The seam credential file exists because the gate must never touch the per-user keychain, not because a native-store failure has a fallback. Four of the mechanisms were forced by observation rather than design: a wrong key stops the child against one endpoint and leaves it healthy against another, so the loss rule keys only on exit or a missed deadline; both E5 cells left an embedded PostgreSQL behind, which is the `SIGTERM`-first stop and the postmaster reap; the crash reason turned out to be on standard error while every log line is on standard output, which is why the tail takes both; and a provider quoting the key back in its own error text is why redaction sits on every message the daemon lifts.

`ponytail:` deliberate ceilings stand with their upgrade paths in the feature decisions: three credential pins instead of the `keyring` facade, which drags its CLI dependencies (the facade the day it stops); the `--help` marker is the whole capability check (a `--version` when a later line adds one); one item per retain, with no batch, entities, temporal window, or type filter (each a request field behind the same route if evaluation evidence asks for it); one provider, one key, one row (a second endpoint or a reranker is a column set behind the same `PUT`); one constitution wording (a template the day a per-market variant is needed); and on Windows no signal reaches the child, so a crash's PostgreSQL waits for a later `pg0 stop` ([SPEC §18](SPEC.md#18-implementation-deferred-contracts)).

**Contract:** [SPEC §4.4](SPEC.md#44-configuration-scopes), [SPEC §4.6](SPEC.md#46-daemon-startup-and-shutdown), [SPEC §5.1](SPEC.md#51-workspace-ownership), [SPEC §5.2](SPEC.md#52-creation-and-recovery), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), [SPEC §16](SPEC.md#16-memory-and-skills), and [SPEC §17](SPEC.md#17-verification).

## Daemon foundation

### D77 — The first milestone settles the daemon's boot contract: one daemon per data root, an embedded-migration store, one error envelope, and a harness that only spawns binaries

**Decision:** Milestone R0 settles the mechanics every later feature group inherits; the full record is [`features/r0-workspace-desk-identity/`](features/r0-workspace-desk-identity/SPEC.md). **Discovery:** `marketrig` finds the daemon through `runtime/endpoint.json` plus authenticated health whose reported UUID must equal the file's, and fails with `DAEMON_UNREACHABLE` and exit `3` when none is reachable; it never spawns `marketrigd`, which the operator starts and the desktop shell takes over with the desktop milestone (D66). **Single instance:** one daemon serves one data root, enforced by an exclusive standard-library advisory lock on a file under `runtime/`, taken before anything is published and released by the operating system on any process death; a second start exits nonzero naming `ALREADY_RUNNING` having written nothing, and scoping the lock to the data root rather than the machine is what lets the acceptance harness run concurrent daemons in scratch roots (D75). Startup runs one fixed order — roots, lock, migrations, daemon UUID, recovery, interrupted-desk completion, bind and credential, endpoint file — so a daemon a client can reach is a daemon whose own state is already resolved. **Schema:** the daemon embeds its ordered migration list, applies what is pending before recovery, and stamps the result in `PRAGMA user_version`; a database newer than the binary stops startup with `DATABASE_NEWER`. There is no bookkeeping table, no external migration file, and no down migration. **Evidence:** the installation-wide append-only `operational_events` table D71 later tails is born here, its rows committed inside the transaction of the change they evidence and its `kind` a closed vocabulary later milestones extend by migration; there are no per-domain event tables. **Errors:** every daemon error crosses REST as one `{code, message}` envelope carrying a stable SCREAMING_SNAKE code and an English message, which the CLI passes through as `error: <CODE>: <message>` or, under `--json`, verbatim, over one fixed exit-code set — `0` success, `1` daemon-reported error, `2` usage, `3` no usable daemon — that later groups inherit and never redefine. **Shutdown:** `POST /quit`, the route D66 gives the desktop, is authenticated and present from R0; it drains and exits within a bounded wait, Ctrl+C on a terminal-attached daemon takes the same path, and a hard kill is left to the next start's recovery. **Acceptance:** the harness is its own internal workspace crate carrying the gate and experiment targets and spawning the shipped binaries rather than linking the daemon. R0's crates are pinned exactly — `axum`, `rusqlite` with SQLite bundled, `uuid` v7, `clap`, `ureq` configured with no environment proxy and redirects disabled, `tracing` with a daily appender keeping seven files, and `sysinfo` for reaping's command-line identity check — the lifetime lock uses the standard library and no crate, and `utoipa` is chosen for OpenAPI emission but stays unwired until a generated client needs it.

**Rationale:** Each of these is cross-cutting: defined with the first endpoint they are inherited, left to each feature group they fork by command family. A daemon the CLI could spawn would be a sole writer with no owner to stop it and a nondeterministic variable in every gate run, and explicit failure matches the product rule that nothing is silently repaired. An operating-system lock releases on any death including a hard kill, so no stale-pidfile heuristic is needed and the standard library covers it without a crate; `user_version` is SQLite's own slot for exactly that number, so a bookkeeping table would duplicate it, and embedding the list keeps binary and schema one release unit (D53). Founding `operational_events` now means later milestones append kinds rather than migrating scattered event rows into the table D71 already commits to tailing. `POST /quit` exists this early because the evidence line needs a deterministic cross-platform stop and Windows offers no signal a harness could portably send. A harness inside the daemon's own test tree could reach private surfaces, which would make D67's public-surfaces-only rule a discipline; a separate crate that can only spawn binaries makes it structural. Every crate is the boring maintained default for its one concern, verified against current documentation on 2026-09-01 and carried green through macOS and Windows CI.

**Contract:** [SPEC §4.3](SPEC.md#43-local-single-user-boundary), [SPEC §4.6](SPEC.md#46-daemon-startup-and-shutdown), [SPEC §5.2](SPEC.md#52-creation-and-recovery), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), and [SPEC §17](SPEC.md#17-verification).

## Desktop and approval controls

### D82 — The desktop milestone gives the user a control plane: three policy columns and approvals on the rows that already exist, one event tail behind origin-checked first-frame sockets, a four-command Tauri shell over a generated-client frontend, and a packaged smoke from a wiped root

**Decision:** Milestone R5 settles the two approval policies and their lifecycles, the approvals listing and decision, the live-event tail and browser-grade socket authentication, the Tauri shell, the Vue frontend, and their verification; the full record is [`features/r5-desktop-approval-controls/`](features/r5-desktop-approval-controls/SPEC.md) (R5-1…R5-8). **Policies:** one row (`id = 1`) in `installation_settings` — `trigger_code_policy` and `paper_order_policy` (`ALWAYS_ALLOW | REQUIRE_APPROVAL`, defaults **Require approval** and **Always allow**), `delivery_mode` with a `CHECK` admitting only `QUEUE`, and `updated_at_ns` — behind `GET`/`PUT /settings/policies`, which reports `steer_available: false`, refuses `STEER` as `409 STEER_DISABLED` and any other unknown value as `400 VALIDATION`, and writes the row with one `POLICY_CHANGED {field, from, to}` per field that actually changes, in one unit; the unit that creates a snapshot or accepts an order reads its column inside its own transaction with no cache, and a policy change never re-gates or ungates a record that already exists. **Approvals:** no table — `code_snapshots` and `trading_actions` each gain `approval` and `decided_at_ns` (null exactly while `PENDING`), `GET /approvals?state=PENDING|DECIDED|ALL` is one union query listing both kinds installation-wide newest first with `{kind, id, desk_id, desk_name, approval, requested_at_ns, decided_at_ns, detail}`, `GET /approvals/{id}` adds a snapshot's source, and `POST /desks/{d}/approvals/{id} {"decision"}` resolves the id against both tables under that desk (`404 APPROVAL_NOT_FOUND`, `409 APPROVAL_DECIDED`), appending `APPROVAL_REQUESTED` where the pending record is born and `APPROVAL_DECIDED` where it is resolved. **Trigger code:** a pending or denied snapshot writes `triggers.next_occurrence_ns` null at every site that computes the projection — create, patch, enable, disable, the scheduler's advance — so the scheduler's one query needs no join; approval recomputes the projection from the decision instant and signals the scheduler (a recurring trigger takes its next boundary, an elapsed one-off stays null), denial keeps the denied snapshot on the trigger, which is freed only by a patch that attaches different code or detaches it, and an identical fingerprint keeps the snapshot and its state. **Paper orders:** a gated submit validates form, attribution, and readiness as ever, inserts the row `PENDING` with no outcome, and answers `202` without starting or consulting the node, replaying `200` on a repeated `action_id`; approval starts the node lazily (`503 MARKET_UNAVAILABLE` leaves the row pending) and runs the ordinary accept path from the stored request with `client_order_id = action_id`, so a post-approval sandbox refusal is recorded as the sandbox's own refused order; denial stores `{"failure_code": "DENIED"}`; a cancel naming a still-pending submit answers `409 ORDER_PENDING_APPROVAL`; `GET /desks/{d}/history/actions` and `marketrig history actions` are the agent's read of its own gated orders, `submit_order` returns the same record, and the seeded constitution names the boundary for desks created from R5 on. **Events and sockets:** the store pulses one post-commit `Notify`; one publisher per daemon holds the installation's `(occurred_at_ns, id)` cursor, wakes on that pulse or every 5 s, reads pages of 500, and fans out to each subscriber's 1 000-frame queue, closing an overflowing subscriber `4408`. `WS /events` takes `{"bearer", "after"?}` as its first frame within 5 s (`4401`, `4400`), subscribes before it reads, replays to the tail position at subscription, answers `{"tail"}`, then streams one frame per row; `GET /events` and `marketrig desk events` are its listing. Every WebSocket checks a present `Origin` against exactly `tauri://localhost`, `http://tauri.localhost`, and `http://localhost:5173` (`403 ORIGIN_REFUSED`), one middleware extends that allowlist to REST with a `204` preflight answered before the bearer check, and the terminal socket gains the same first-frame path with `4404`/`4409` as close codes while its header path is untouched. **Shell:** `src-tauri/` is the workspace crate `marketrig-desktop` (main binary `marketrig-desktop`, never `MarketRig`, which collides with the CLI sidecar on a case-insensitive filesystem), with `marketrigd`, `marketrig`, and `marketrig-mcp` as bundled sidecars and four commands — `read_endpoint`, `start_daemon` (detached, 30 s for a new daemon UUID, else `DAEMON_START_FAILED`), `set_tray_pending(n) -> () | Err(text)`, and `exit_app` — and no HTTP of its own; Close prevents destruction and hides, `ExitRequested` is prevented, the tray is *Open* / a disabled pending line / *Quit*, Quit is the webview's `POST /quit` then health polling then `exit_app` from either entry point, the single-instance callback focuses the window, and autostart is the plugin with `--hidden`. **Frontend:** the root Vue 3 application over the client generated from `marketrigd --openapi`, four composables (`useDaemon`, `useEvents`, `useTerminal`, `useApprovals`), three panels with the five-tab right panel, one `@theme` token block, notifications only while hidden or unfocused for actionable kinds, and every prose string through the `en` catalog. **Schema:** migration 6 adds `installation_settings`, rebuilds both approvable tables backfilling `ALWAYS_ALLOW` with `decided_at_ns = created_at_ns` and dropping `code_snapshots.approved_at_ns`, and widens `operational_events.kind` with `POLICY_CHANGED`, `APPROVAL_REQUESTED`, and `APPROVAL_DECIDED`. **Acceptance:** no new helper and no attended scenario — G38–G41 extend the chain after G37 (policies and the tail, trigger-code approval, paper-order approval, sockets and a hard kill), G21's prologue and E3's setup put trigger code on **Always allow** first because the installed default gates it, CI gains a `frontend` job on both platforms that regenerates the client and fails on a diff before Prettier, ESLint, `vue-tsc`, and Vitest, and `pnpm smoke` drives the packaged application through WebdriverIO's Tauri service in embedded-driver mode, operator-run once per platform from a root it wipes only under `MARKETRIG_SMOKE_WIPE=1`. Pins: `utoipa =5.5.0` and `utoipa-axum =0.2.0`; `tauri =2.11.5`, `tauri-build =2.6.3`, `tauri-plugin-single-instance =2.4.4`, `tauri-plugin-notification =2.4.0`, `tauri-plugin-autostart =2.5.1`, `tauri-plugin-log =2.9.1`, and `tauri-plugin-wdio-webdriver =1.3.0` behind the `wdio` feature; `vue =3.5.42`, `vite =8.2.2`, `typescript =6.0.3`, `tailwindcss =4.3.3`, `reka-ui =2.10.4`, `vue-i18n =11.4.10`, `@xterm/xterm =6.0.0` with its fit, WebGL, and Unicode 11 addons, `@hey-api/openapi-ts =0.99.0` with its bundled fetch client, `vitest =5.0.0`, `@wdio/cli =9.31.5`, and `@wdio/tauri-service =1.3.0`, on Node.js `24.18.0` and `pnpm =11.25.0` through Corepack.

**Rationale:** Each clause makes a root sentence literal without adding a product entity. The approval lifecycle is the rows that already exist because D70 said the approval *is* the record; folding eligibility into the projection is what keeps enable, disable, and an elapsed occurrence from resurrecting an unapproved trigger with no scheduler change at all. Validating an order's form before it goes pending means the user is never shown a request the daemon would have refused anyway, and replaying the stored request on approval is what makes an approved order indistinguishable from an ungated one, refusal included. First-frame authentication and the exact-origin allowlist exist because the browser WebSocket API cannot send a header and D44 forbids a credential in a URL; the same allowlist had to reach REST, because the webview calls this loopback API cross-origin and the browser preflights. Keeping the shell out of HTTP leaves one verifier and one client, and spawning the daemon detached is what keeps a shell crash from taking the desk down with it. Four mechanisms were forced by observation rather than design: the main binary's name collided with the CLI sidecar on a case-insensitive filesystem; the WebDriver service's per-command focus poll waits on a plugin the shell does not carry, so the spec switches windows once; a WebDriver click opens a Reka `Select` and its pointer-up closes it again; and a standalone-pnpm Windows box carries no `pnpm.cmd`, so the build invokes the Tauri CLI through Node directly.

`ponytail:` deliberate ceilings stand with their upgrade paths in the feature decisions and slice 008: two approvable kinds behind one route resolved by trying both tables (a third kind is a third arm of the same match); the withheld order request is replayed from stored JSON with no queue, worker, or timer, so an order nobody decides stays pending forever (expiry is a column if evidence asks for it); the tray count is a menu line and a title, not a badge (a dock badge is one call behind the same command); tray labels are English until R6 rebuilds them through `set_locale`; the packaged smoke drives a bundle that differs from the shipped one by the feature-gated embedded WebDriver server, which is the price of having no `tauri-driver` install step and no platform branch; and close-hides itself is confirmed by hand rather than by the smoke, because a WebDriver close destroys the window instead of requesting one and the capability grants no `allow-close`.

**Contract:** [SPEC §4.3](SPEC.md#43-local-single-user-boundary), [SPEC §4.4](SPEC.md#44-configuration-scopes), [SPEC §6.5](SPEC.md#65-terminal-manager), [SPEC §8.3](SPEC.md#83-trigger-lifecycle), [SPEC §10](SPEC.md#10-scheduled-trigger-semantics), [SPEC §12.3](SPEC.md#123-trading-actions-and-approvals), [SPEC §13.1](SPEC.md#131-mcp-trading-plane), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §14](SPEC.md#14-desktop-and-application-lifecycle), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), and [SPEC §17](SPEC.md#17-verification).

## OpenViking continuity

### D83 — OpenViking replaces Hindsight and owns both memory and skills: an operator-named Python and Node, an offline venv from bundled wheels, one `openviking-server` child, one user per desk, the vendored plugins behind a per-desk credential file, and `.agents/skills/` as a read-only projection

**Decision:** OpenViking replaces Hindsight as the owner of a desk's experiential memory *and* its procedures; the full record is [`features/openviking-continuity/`](features/openviking-continuity/SPEC.md) (OV-1…OV-7). **Removal:** every Hindsight mechanism is deleted outright with no import, shim, or coexistence — the `memory_child` row and its discovery, the derived banks, the four `marketrig memory` commands and their routes, `memory-standin`, G33–G37, and E5. **Setup:** one installation row, `openviking_setup`, holds two operator-named absolute paths, each validated by running it — Python must print exactly `3.12`, the minor upstream builds and tests its wheels on, and Node major `>= 22`, the runtime the Codex plugin names — and `GET /openviking/candidates` offers a fixed per-platform discovery list the same validators accept or refuse; provisioning replaces `<data root>/openviking/venv` and installs `openviking==0.4.17.1` with `pip --no-index --find-links` from the per-platform wheel directory the release unit bundles as `openviking-wheels/<platform>/`, whose committed lockfile is generated on its own platform because pip evaluates environment markers against the host, so the install needs no network and no compiler. **The child:** one `openviking-server` starts with the daemon — after recovery, before the listener accepts desk work — under the containment primitive on a daemon-picked loopback port, configured by a daemon-written `ov.conf` whose two secrets are `${…}` references the loader expands from the child's environment, and is ready on `GET /ready`, the probe that exercises storage, key manager, and a live embeddings call, so a provider the endpoint refuses never reaches `READY`. The child exiting or the 120 s deadline passing is `OPENVIKING_LOST`, sets the row `UNAVAILABLE` with the last output line, and drops the desk keys; there is no automatic restart, `POST /openviking/retry` starts it once more, and a lost child never blocks desks, triggers, trading, or sessions. **Tenancy:** `api_key` mode behind a per-start root key held in memory, account `marketrig`, one user `desk-<uuid hex>` per desk, each key obtained at every readiness from OpenViking's own seeded-key route and therefore deterministic in one installation secret, `openviking_seed`, which lives in the credential store — so a Retry hands a live session the same key it already has. **Credential delivery:** the plugins read one per-desk plain-text `ovcli.conf` (`url`, `api_key`, `account`, `user`), written `0600` by the daemon at every launch under `<data root>/openviking/plugin/<desk-id>/`, located by seven path-only variables (`OPENVIKING_HOME`, `OPENVIKING_CLI_CONFIG_FILE`, `OPENVIKING_CONFIG_FILE` naming an absent file so `~/.openviking/` is never read, `OPENVIKING_MEMORY_ENABLED`, and the three plugin state directories) that ride the runtime process environment for Claude Code and, for Codex, are prefixed onto each hook command and set on the MCP entry's `env` table — because the Codex app-server runs every desk's hooks and MCP servers with no per-desk environment (found by E6, 2026-09-08). No secret is on any process environment or in any runtime-owned file; without a desk key the launch carries `OPENVIKING_MEMORY_ENABLED=0` and no registration. **The plugins:** OpenViking's Claude Code and Codex memory plugins are vendored unmodified at one pinned commit, seeded per desk into `<workspace>/.marketrig/plugins/`, reconciled byte for byte at startup like the `CLAUDE.md` shim, and registered through the launch files the daemon already writes; they are the capture and recall path and their MCP tools are the agent's memory plane. **Skills:** a desk's skills are canonical in OpenViking under its user, and `.agents/skills/` is a MarketRig-owned read-only projection (files `0444`, directories `0555`, read-only attributes on Windows) rewritten before every activation, after every `SESSION_TURN_ENDED`, and whenever a skill write lands; the `.claude/skills` link stands unchanged, a projection failure leaves the last tree and never blocks an activation, and a direct edit is refused by the filesystem and named in the constitution. OpenViking's MCP `write` and `edit` refuse the managed `skills/` subtree, so the write path is the continuity CLI: `marketrig [--json] skill put <desk> --file <SKILL.md>` and `skill delete <desk> <name>`, which the daemon forwards under the desk key with a 90 s write bound and a 120 s CLI ceiling, because OpenViking summarizes and embeds a skill through the provider before answering, and projects once before it replies. The seeded `desk-improvement` skill is uploaded into the desk's user behind the creation request rather than inside it, projecting itself when it lands; `session_skill_extraction_enabled` stays `false`, so only the agent writes skills. **Schema:** migration 7 drops `memory_child`, creates `openviking_setup`, deletes every `MEMORY_*` event, and rebuilds the kind vocabulary with `OPENVIKING_CONFIGURED`, `OPENVIKING_PROVISIONED`, `OPENVIKING_STARTED`, `OPENVIKING_LOST`, `OPENVIKING_UNAVAILABLE`, `DESK_MEMORY_PROVISIONED`, `SKILLS_PROJECTED`, and `SKILLS_PROJECTION_FAILED`; migration 8 adds the measured `memory_provider.embedding_dimension`. The child's live state, the root key, and the desk keys are memory only, and the child is a `children.json` record recovery's first step already reaps. **Acceptance:** `openviking-standin` replaces `memory-standin` and is registered through the setup route's test seam; O1–O6 replace G33–G37 and R5's four scenarios are renumbered O7–O10; E6 replaces E5 and skips with evidence unless the operator names the Python, the Node, the wheels, and a provider. Delivered on macOS 2026-09-08 (gate O1–O10 green, both E6 cells passing on real OpenViking 0.4.17.1 behind OpenRouter); the Windows E6 pair is outstanding and is R6's first entry check.

**Rationale:** Two stores nothing connects made the same experience have to be retained twice, and the retain step depended on the agent remembering to call a CLI, which E5 showed happens only when the constitution says so. OpenViking keeps memories and skills in one per-user context store with one retrieval surface and captures sessions through its own runtime plugins, so continuity accumulates without agent bookkeeping. With no released users, removal beats a compatibility path that would cost a second memory system in every scenario. The user chose user-installed prerequisites over bundled runtimes, so validation by running the named executable — R3's shape — replaces D47's portable CPython, and `--no-index` is what makes "bundled wheels" a property the checks assert. Start-with-daemon and no automatic restart were settled by the user; `/ready` rather than `/health` is the readiness the plugins need because it is the endpoint that embeds. The seed is what makes "regenerate the root key every start" and "never persist a desk key" compatible with sessions that outlive a restart. Three mechanisms were forced by observation rather than design, all in the macOS E6 cells: the skill write is a provider round trip, so its 15 s bound became 90 s; the seed upload blocked desk creation past the client's ceiling, so it moved behind the request; and the Codex app-server's shared, per-desk-blind environment moved the credentials out of the process environment into the per-desk file the plugins' own resolvers already prefer.

`ponytail:` deliberate ceilings stand with their upgrade paths in the feature decisions: one Python minor and one venv location (a wider validation matrix behind the same setup route; the install is uv's from the bundled directory since 2026-09-08, pip having measured 13–17 minutes on Windows); a fixed 120 s readiness deadline and one config layout (a measured deadline when a real machine misses it); one account and one flat user per desk, with peers, workspaces, and shared `viking://agent/skills` unused; one vendored plugin commit, upgraded as a seed change the reconcile check verifies; a full rewrite of the projection on every refresh (a content-hash diff when a desk's skill set makes it measurable); and `SKILL.md` alone on the write path, auxiliary files arriving with the dict form of `data`. Three ceilings are outside MarketRig's code: the Claude plugin writes a hard-coded `~/.openviking/last_inject.md` and probe cache, which stands because the vendored plugins are unmodified; a Windows box under a deep data root needs `LongPathsEnabled` because `volcengine-python-sdk` ships wheel members past the legacy `MAX_PATH` ([SPEC §18](SPEC.md#18-implementation-deferred-contracts)); and Codex exposes no daemon-visible turn end, so the turn-end projection refresh fires only on Claude's `Stop` hook ([SPEC §18](SPEC.md#18-implementation-deferred-contracts)).

**Contract:** [SPEC §2](SPEC.md#2-canonical-terminology), [SPEC §4.1](SPEC.md#41-installation), [SPEC §4.4](SPEC.md#44-configuration-scopes), [SPEC §4.6](SPEC.md#46-daemon-startup-and-shutdown), [SPEC §5.1](SPEC.md#51-workspace-ownership), [SPEC §5.2](SPEC.md#52-creation-and-recovery), [SPEC §7](SPEC.md#7-activation), [SPEC §13.2](SPEC.md#132-cli-continuity-plane), [SPEC §14](SPEC.md#14-desktop-and-application-lifecycle), [SPEC §15](SPEC.md#15-persistence-crash-recovery-and-history), [SPEC §16](SPEC.md#16-memory-and-skills), and [SPEC §17](SPEC.md#17-verification).
