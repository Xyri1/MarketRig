...
MVP platforms are Windows and macOS on Apple Silicon. MVP agent runtimes are Codex CLI and Claude Code. MVP trading is NautilusTrader sandbox and paper execution only, equities only — US, Hong Kong, and China A-share — with Kraken crypto deferred past MVP (per D84). `marketrigd`, `marketrig`, and `marketrig-mcp` are Rust binaries on rustc 1.98 or newer with edition 2024. MarketRig ships no interpreter: the operator names the Python 3.12 and the Node the supervised OpenViking child and its plugins run on (§4.1).
...
A durable desk-bound `marketrigd` job that fires from its schedule or from a direct invocation, optionally runs an immutable code snapshot, and produces a result for the desk agent.

### Invocation

A local producer's request to fire one named trigger in one desk, carrying a caller-supplied request identity and optional raw input. Accepted, it is one firing of that trigger; it is not a separate user-managed job and there is no event name to match.
...
Agent-owned subjective and experiential memory, held in that desk's own OpenViking user and captured from its sessions by the seeded plugins. It is not authoritative trading reality.

### Skill

Durable procedural guidance owned by the desk's OpenViking user and projected read-only into the workspace, where both supported runtimes load it within that desk (per D83).
...
Tauri 2 / Vue 3          marketrig CLI          Codex CLI / Claude Code
      ...
      +--- authenticated loopback REST + WebSocket --------+
                          ...
                          └── supervised OpenViking child -> one user per desk
```
...
- Agent-facing trading semantics never leak NautilusTrader APIs; the agent's memory surface is the seeded OpenViking plugin's own MCP tools, and the child's topology, credentials, endpoint, account, and user identifiers stay private to `marketrigd` (per D83).
...
- the locked per-platform wheel set `openviking-wheels/<platform>/`, bundled as a resource and installed offline into the daemon's own virtual environment for the supervised OpenViking child (per D47, D65, D83).

MarketRig ships no interpreter of its own: the operator names an absolute Python 3.12 and an absolute Node ≥ 22, each validated by running it (§4.4), and nothing but the memory child and its plugins depends on either. No build toolchain is required at run time, and provisioning is offline by construction. The Tauri 2 bundler produces a per-user NSIS installer for Windows and a `.app` distributed in a DMG for Apple Silicon macOS. Node.js, pnpm, and Corepack are build-time only.
...
- That framework must emit an OpenAPI document from its own route definitions; a handwritten document is not acceptable input, because generation exists to remove a duplicated contract (per D59). The emitter is `utoipa` with its axum integration, wired since the desktop milestone (per D82): every HTTP handler carries its path annotation on one router whose split gives the served router, and `marketrigd --openapi` prints that document and exits without touching a data root. The three WebSocket routes are outside it, because an upgrade cannot be described — the terminal and channel sockets are absent and `/events` appears only as its listing.
...
- REST requests require that credential. WebSocket connections validate an exact allowed origin at the handshake and authenticate with a first-frame bearer credential before any application message is accepted, because the browser WebSocket API cannot send headers and credentials never appear in URLs (per D44, D66); REST requests carrying a browser `Origin` are answered under that same allowlist, preflight included.
- Concretely (per D82), the allowlist is exactly `tauri://localhost`, `http://tauri.localhost`, and `http://localhost:5173` — the macOS and Windows webview origins and the dev server's. A present `Origin` outside it is `403 ORIGIN_REFUSED` before the upgrade and before the route; a request carrying no `Origin` — the CLI, the MCP adapter, the Channel bridge, the harness — passes untouched and gains no CORS header. An allowed origin's `OPTIONS` preflight is answered `204` ahead of the bearer check, with the echoed origin, the methods and headers the frontend uses, a bounded max age, and `Vary: Origin`, which every other answer to an allowed origin also carries. A browser socket authenticates in one text frame within 5 seconds — `{"bearer": "<credential>", "after": "<cursor>"?}` — closing `4401` on a missing or wrong bearer and `4400` on an unparseable frame, and every close code carries the same SCREAMING_SNAKE machine code as its reason text, because a browser hides handshake statuses from script. A client that can send a header still authenticates with one.
...
- The frontend REST client is generated from the daemon's OpenAPI document with exactly pinned `@hey-api/openapi-ts` and its bundled `@hey-api/client-fetch` plugin, committed, and re-checked in CI so client and daemon cannot drift (per D59). Terminal bytes and live events use the browser's native WebSocket API separately.
- The supervised OpenViking child binds only to loopback and runs in `api_key` mode behind a MarketRig-owned root key minted per start; only `marketrigd` holds that key, and only the seeded plugins — under the desk key the daemon writes for them — reach the desk's own data (per D83).
...
The REST surface is the document `marketrigd --openapi` prints from its own routes; the terminal socket's framing is §6.5's and the event socket's is §14's.
...
The memory setup and the memory provider are two more installation resources (per D83). `openviking_setup` is one row with `state IN ('UNCONFIGURED','PROVISIONING','AVAILABLE','UNAVAILABLE')`, the two operator-named absolute paths and the versions they reported, the venv path, `provisioned_at_ns`, and a failure code and message: read through `GET /openviking`, which also answers the child's live state (`NOT_STARTED | STARTING | READY | LOST`) and which `READY` desks are provisioned; offered candidate paths through `GET /openviking/candidates`, a stateless per-platform lookup that writes nothing; provisioned through `PUT /openviking/setup {python, node}`, which validates both paths by running them — Python must print exactly `3.12` (else `PYTHON_UNSUPPORTED` or `PYTHON_PROBE_FAILED`), Node major `>= 22` (else `NODE_UNSUPPORTED` or `NODE_PROBE_FAILED`) — then answers `202` and installs the bundled wheel set offline into `<data root>/openviking/venv`, landing `AVAILABLE` or back at `UNCONFIGURED` with `PROVISION_FAILED` and the last output line, a second request while `PROVISIONING` refused `SETUP_BUSY`; and started again from `UNAVAILABLE` through `POST /openviking/retry`. `memory_provider` is one row with `base_url`, `llm_model`, `embedding_model`, the measured `embedding_dimension`, and an opaque key reference, written by `PUT /memory/provider {base_url, api_key?, llm_model, embedding_model}`; an omitted `api_key` keeps the stored one, the save measures the embedding dimension once with a live request and answers `PROVIDER_REJECTED` without writing when that fails, every answer carries `api_key_present` and never a key, and a provider change restarts the child with the new configuration (§16).

Trigger-code authorization, paper-order approval, and trigger-delivery mode are one installation resource (per D70). Code defaults to **Require approval**, orders to **Always allow**, and delivery to `QUEUE`; `STEER` is refused as a disabled mode and any other value as invalid. Each changed field appends one installation-wide policy-changed record. Policies and approvals are reachable only through REST and the desktop: no `marketrig` command reads a policy, lists approvals, or decides one. Concretely (per D82), migration 6's one-row `installation_settings` holds `trigger_code_policy` and `paper_order_policy` (`ALWAYS_ALLOW | REQUIRE_APPROVAL`) and `delivery_mode` (a `CHECK` admitting only `QUEUE`) beside `updated_at_ns`; `GET /settings/policies` answers the three columns plus `steer_available: false`, and `PUT /settings/policies` takes any subset, refusing `delivery_mode: "STEER"` as `409 STEER_DISABLED` and any other unknown value as `400 VALIDATION`, then writes the row and one `POLICY_CHANGED {field, from, to}` per field that actually changes, in one unit, and answers the resource. The unit that creates a code snapshot or accepts an order reads its own column inside its own transaction, with no cache, and a policy binds only what is created after it: a pending record stays pending until a person decides it, and a record the policy never gated is never re-gated.

Provider secrets live only in the native per-user credential store — macOS Keychain or Windows Credential Locker — reached by `marketrigd` alone through one maintained, exactly pinned binding that names the platform backend explicitly rather than relying on backend discovery: `keyring-core =1.0.0` with the platform store crate set as the default store at daemon start — `apple-native-keyring-store =1.0.2` on macOS, `windows-native-keyring-store =1.1.0` on Windows — under service `marketrig` and one account per secret, skipping the `keyring` facade because it drags its own command-line dependencies (per D81). SQLite stores only non-secret configuration and opaque references. Native-store failure surfaces as an unavailable capability — `CREDENTIAL_STORE_UNAVAILABLE`, with nothing written — and no plaintext, environment-file, or second-vault fallback. Secrets never appear in SQLite, logs, prompts, URLs, desk files, trigger results, or CLI output (per D49). Under the test seam alone (§17) the store is a 0600 `runtime/credentials.json` inside the relocated root, so no acceptance run touches the per-user credential store; it is the harness seam and never a fallback (per D81).

There are exactly two recorded exceptions, both daemon-owned and neither on a runtime process environment, stated wherever they are specified (per D49, D83): the supervised OpenViking child receives the provider key and the daemon's per-start root key through its own process environment, which its configuration loader expands from `${…}` references so the file on disk holds no secret; and each desk's OpenViking key is written plain into that desk's daemon-owned `ovcli.conf`, mode `0600` under `<data root>/openviking/plugin/<desk-id>/`, because the vendored plugins read their credential from that file (§16).

For the memory child's hosted models the user sets one OpenAI-compatible base URL, API key, VLM model, and embedding model; the same endpoint and key serve both, and no reranking model is configured. SQLite persists the base URL, the selected models, and the measured embedding dimension; the key stays in the credential store. Both model ids are typed by the user; `GET /memory/provider/models` still lists the provider's models live, never persisted or cached, with a fetch failure explicit — `PROVIDER_UNREACHABLE` — rather than a stale list, but no MarketRig surface drives a selector from it. There is no embedding-model lock: the dimension is measured at save time and written into the child's configuration, and OpenViking's local vector store carries its own dimension check (per D18, D83).
...
The setting is migration 11's nullable `installation_settings.locale` (`'en' | 'zh-Hans'`, `NULL` until written), read and written through `GET`/`PUT /settings/locale` and read by the daemon for nothing else. The webview detects once: after health, a stored value is applied; `NULL` is resolved by BCP 47 lookup over `navigator.languages` with CLDR likely subtags (`Intl.Locale.maximize()`, then right-truncation against the shipped catalogs; no hit is `en`), written, then applied, and a later change in Settings wins over a detection still in flight. `applyLocale` is the one function startup and the Settings **Language** select call: vue-i18n's global locale, `document.documentElement.lang`, then the shell's `set_locale`. Gate L1 proves the agent-facing byte-identity under both locales (§17). Catalog mechanics, fonts and input methods, and the parity checks are in `sdd/features/localization/SPEC.md`.
...
6b. start the OpenViking child when openviking_setup is AVAILABLE and the provider row is complete (§16);
    readiness is awaited off this path, so startup never waits on it and a failure never blocks a desk
...
├── canonical skills in its OpenViking user, projected read-only into .agents/skills
├── OpenViking user (memory)
├── selected runtime
...
- MarketRig owns runtime compatibility material — the shim, the seeded OpenViking plugin trees under `.marketrig/plugins/`, and the `.agents/skills/` projection — and seeds the desk constitution from its own texts, which ship in the daemon's source tree under `crates/marketrigd/seed/` with the desk name substituted (§16, per D81, D83).
- After the desk first becomes `READY`, the agent owns research, tools, experiments, and its OpenViking memory and skills; MarketRig never rewrites agent-owned workspace material and reconciles only its own shim, the seeded plugins, and the link.

A desk's canonical skills live in its OpenViking user; `.agents/skills/` is MarketRig's read-only projection of them (§16, per D83). Claude Code reaches that same physical directory through `.claude/skills`: a relative symlink to `../.agents/skills` on macOS and a directory junction to the absolute workspace path on Windows. MarketRig maintains no second, copied skill tree and never replaces an ordinary directory found at `.claude/skills` (per D21). Startup reconciles that link, the `CLAUDE.md` shim, and the seeded plugin trees byte for byte, and nothing else (per D81, D83): a missing link is recreated, creating an empty `.agents/skills/` first when it too is gone; a link pointing anywhere but the desk's own `.agents/skills` is repointed; an ordinary file or directory there is left in place and named in the derived workspace-status reason (§5.2).
...
`marketrigd` persists a `CREATING` desk row before touching its workspace. It bootstraps `AGENTS.md`, the exact `CLAUDE.md` shim, the seeded OpenViking plugin trees under `.marketrig/plugins/`, an empty `.agents/skills/`, and the `.claude/skills` link idempotently — in that order, each step skipped when its target already exists, so a retried creation completes without rewriting (per D81, D83) — then marks the same row `READY`. The improvement skill is no longer a workspace file: it is uploaded into the desk's OpenViking user behind the creation request and reaches the workspace through the projection (§16). A failure preserves the row and partial workspace as `FAILED` with a failure code and message; retry reuses its UUID, name, and path. Creation answers only once the row is `READY` or `FAILED`, so a client observes `CREATING` only after a crash, and startup completes every interrupted `CREATING` desk before accepting work.

Readiness covers only the durable row and the local workspace. The desk's OpenViking user and its seeded skill, trading-node and paper-book setup, runtime activation, and trigger work stay lazy and keyed by the desk UUID; a desk created while the child is `READY` is provisioned behind its creation request, and one created while it is not is provisioned at the next readiness.

For a `READY` desk, startup validates the workspace without recreating or rewriting agent-owned material, reconciling only the `CLAUDE.md` shim, the seeded plugin trees, and the `.claude/skills` link (§5.1); a desk created before the constitution was seeded keeps its own `AGENTS.md` and gains only the MarketRig-owned material and that link (per D81, D83). A missing or unusable workspace, or an unreadable `AGENTS.md`, produces a derived workspace-unavailable status — computed at read time with a one-line reason, never stored, and carried only by a `READY` desk — while the durable desk stays `READY`, and blocks neither other desks nor daemon startup.
...
Concretely (per D80), the crate is `portable-pty =0.9.0`: its Unix spawn is `std::process::Command` with a `pre_exec` that only calls `setsid` and `TIOCSCTTY` before `exec`, so no single-threaded spawn helper exists (per D31). The ring is 256 KiB, replayed as one binary frame; shutdown drains for at most 2 seconds, then terminates the tree — `killpg` on the child's own session on macOS, a kill-on-close Job Object the terminal assigns immediately after spawn on Windows — and the exit is the child's wait, never the reader's EOF. The attachment is `GET /desks/{desk_id}/terminal`, a bearer-authenticated WebSocket answering `404 DESK_NOT_FOUND`, `409 NO_LIVE_SESSION`, and `400 VALIDATION` for a non-upgrade before any attachment is taken: binary frames are raw bytes both ways, the client's text frame `{"resize":{"cols":n,"rows":n}}` resizes, a new attachment closes the previous one `4001`, a consumer whose 1 MiB send buffer fills is dropped the same way, and process end sends `{"exited":{"reason":…,"code":…}}` then close `1000`. One byte is interpreted, and only one: ConPTY writes the cursor-position query `ESC[6n` at spawn and holds the child's console initialisation until the host answers, so with no attachment live the reader answers `ESC[1;1R` itself and drops the query from the ring; with an attachment live the bytes pass through and the viewer's terminal answers, as on macOS. It is the sole exception to raw-byte pumping, and it is not presentation (per D30). A browser opens the same route without an `Authorization` header (per D82): the handshake still checks §4.3's origin allowlist, the connection upgrades, the first text frame must carry the bearer inside 5 seconds or it closes `4401`, and the pre-attachment checks then answer as close codes — `4404 DESK_NOT_FOUND`, `4409 NO_LIVE_SESSION` — before any attachment generation is taken, so a refused or unauthenticated connection never supersedes a live viewer. The header path is unchanged.

Each attachment first sends `{"attached":{"terminal_id":"<UUIDv7>","offset":"<decimal byte offset>","replay_bytes":n,"cols":n,"rows":n,"windows_pty":null}}`, then the ring (if nonempty), then live bytes. The identity is minted at PTY spawn and stays unchanged across attachments. The offset counts bytes before the retained ring after unattended cursor-query filtering; the counter and ring are captured under one lock. Dimensions are the PTY size at attachment. On Windows, `windows_pty` is `{"backend":"conpty","buildNumber":n}`; the existing sysinfo `kernel_version()` supplies the actual build, or null when unavailable. Warm viewers can discard overlapping replay and detect missing bytes without a daemon-side emulator or transcript.
...
- before every activation, new or resume, the daemon refreshes the desk's `.agents/skills/` projection and writes that desk's `ovcli.conf` and the plugins' registration entries, so a session starts on the skills OpenViking currently holds; a projection that cannot run leaves the last tree in place and never blocks or delays the activation (§16, per D83).
...
There is no `MANUAL` trigger kind and no separate event kind: every trigger may be fired by its schedule, by direct invocation, or both (per D34). Immediate user input goes directly to the active runtime; a user request may cause the agent to create a trigger for later work.

Trigger terminology is exact:

- an **occurrence** is a calendar candidate or a direct invocation that may be accepted, refused, or deduplicated;
...
A missed scheduled occurrence is operational evidence, never a firing. A refused invocation is a daemon log line, never a firing and never buffered input.

### 8.2 Invocation semantics

A local producer invokes one trigger by name or id in one desk through `marketrig` and the authenticated daemon boundary, with a caller-supplied request identity scoped to the desk and the trigger and an optional raw input (per D34; the feature SPEC `event-triggers` §2 is the full contract):

- the duplicate check comes first: an already accepted request answers its original firing whatever the trigger's state has since become, before or after restart;
- a missing, deleted, disabled, consumed, unapproved, or elapsed target is refused with its reason, and nothing is stored to run later;
- input age is not a refusal reason — the producer owns relevance, and acceptance order, not the producer's timestamp, orders the work;
- backlog is never read: pending prompts, firings, and executions do not refuse a distinct invocation, and nothing is dropped or combined;
- a one-off is consumed by its first accepted firing through either entry path; a recurring trigger's schedule and projection are untouched by an invocation;
- the input is data for the agent and for the code's firing document, never executable source.

Public ingress, connectors, and richer filtering are **deferred** (§18).
...
- Concretely (per D82), a snapshot's `approval` is `ALWAYS_ALLOW | PENDING | APPROVED | DENIED` with `decided_at_ns` null exactly while `PENDING`, and the projection rule at every site that computes it — create, patch, enable, disable, and the scheduler's advance — is that `next_occurrence_ns` is non-null only when the trigger is enabled, undeleted, and its snapshot is absent, `ALWAYS_ALLOW`, or `APPROVED`. Approval recomputes the projection from the decision instant and signals the scheduler in one unit; denial leaves the denied snapshot on the trigger, which stays undue until a patch attaches different code or detaches it. A patch whose code fingerprints identical to the current snapshot keeps that snapshot and its state.
- Only a code or launch-snapshot change requires reapproval; brief, schedule, and enablement changes do not.
- Each accepted firing atomically captures the occurrence identity — the scheduled instant or the caller's request identity — the current brief and context, trigger revision, and approved code-snapshot identity as immutable firing-time provenance. The same transaction consumes a one-off or advances a recurring definition. Later trigger edits affect only future firings.
- Disabled triggers do not fire, buffer input, or create missed occurrences.
- Consuming a one-off disables it in the accepting unit, whichever entry path accepted the firing; the row reads disabled and its firing exists. Re-enabling it is explicit consent to fire again: the enable recomputes the projection, and a new distinct invocation is accepted, while the already accepted request stays a duplicate.
- Re-enabling a recurring scheduled trigger resumes at its next future occurrence; a recurring trigger without a schedule fires only when invoked.
- A one-off whose scheduled instant has passed is refused for invocation until explicitly rescheduled or its schedule detached; the schedule owns a passed deadline.
...
- A one-off is atomically consumed at its first accepted firing — the accepting unit sets it disabled and its projection null, whether the schedule or a direct invocation accepted it (§8.3) — even if code later fails or times out.
...
If the desk has no live managed session, MarketRig activates it resume-first and waits for structured-input readiness. If it has a live session, the adapter applies its runtime-private safe queue boundary. Neither path creates a product-level agent status. There is no queue cap and no backlog-based admission control: an accepted occurrence is queued individually, and pending volume never refuses a distinct invocation (per D34; §8.2).
...
Research data is informational only, never a writer of trading state, and reached only through `marketrig` (per D9). For A-share it is the official HiThink service, a hosted keyed REST contract the daemon calls with its own HTTP client and proxies as one allowlisted `GET /research/hithink/{path}` behind `marketrig research hithink` — the design is [`features/hithink-a-share/`](features/hithink-a-share/SPEC.md) (per D84). OpenBB stays deferred past MVP on scope; when it arrives it is hosted as its own supervised child beside the memory child's.
...
- Two feeds serve the equity markets: the keyless Yahoo client for US and Hong Kong, and, behind one operator tick box that a saved key enables, the official HiThink service for China A-share — batched snapshot polling, the exchange's trading-day calendar, and a null source time exposed as such (per D84, [`features/hithink-a-share/`](features/hithink-a-share/SPEC.md) §2–§3). Neither carries a bid or ask, so the desk's equity book is synthesized in every market and everything downstream that would reason about spread or depth is told so; non-US observations may be delayed, which the read's provenance, timing, and age expose rather than hide (per D76).
...
Concretely (per D82), `trading_actions` carries `approval` (`ALWAYS_ALLOW | PENDING | APPROVED | DENIED`) and `decided_at_ns`, null exactly while `PENDING`, and the action record every surface answers carries both, with the outcome absent while pending. Under **Require approval** a submit validates form, attribution, and desk readiness exactly as it does today, inserts the row pending with no outcome, appends `APPROVAL_REQUESTED`, and answers `202` without starting or consulting the node; a repeated `action_id` answers `200` with that record. `GET /approvals?state=PENDING|DECIDED|ALL` lists pending code snapshots and trading actions together, installation-wide and newest first, each item naming its kind, desk, request and decision instants, and detail, and `GET /approvals/{id}` adds a snapshot's source. `POST /desks/{desk_id}/approvals/{id} {"decision": "APPROVE" | "DENY"}` resolves the id against both tables under that desk — `404 APPROVAL_NOT_FOUND`, `409 APPROVAL_DECIDED` — and appends `APPROVAL_DECIDED` in the unit that writes the state. Approving an order starts the desk's node lazily, leaving the row pending under `503 MARKET_UNAVAILABLE`, then runs the ordinary accept path from the stored request with `client_order_id = action_id`, so a sandbox refusal after approval is recorded as the sandbox's own refused order; denial writes the `DENIED` failure code as the outcome and is terminal. A cancel naming an order whose submit is still pending answers `409 ORDER_PENDING_APPROVAL` and records nothing, because there is no sandbox order to cancel and the agent cannot decide the approval. `GET /desks/{desk_id}/history/actions` lists a desk's trading actions newest first with their approval state, which is how an agent learns why its order has not filled. The seeded desk constitution names the boundary for desks created from R5 on; existing desks are never rewritten (per D20).

Paper scope is equities, and MarketRig inherits the sandbox's physics exactly rather than approximating them:
...
- **Kraken crypto is deferred past MVP** (per D84): D74 keeps its design — one margin netting account per desk spanning spot and futures, long and short, the sandbox's single-order types, funding and mark and index as observations only — as the recorded shape for a later milestone, and nothing of it ships in the MVP.
...
Global flags precede the group. Desk scope is explicit or resolved by the daemon from the caller's working directory; the CLI never derives a desk itself, and it resolves a desk name to its UUID through the daemon's own listing. Mutating requests carry the trigger environment as attribution, and nothing else is inferred. The `history` group is `marketrig [--json] history <orders|fills|cycles|actions> <desk-name-or-id>`: name-or-id resolved through the daemon's listing, plain rows newest first, `--json` the route's body verbatim, exit codes unchanged (per D78, D82); `actions` prints each trading action's instant, kind, action id, approval state, and the outcome's status or failure code, which is the agent's read of its own gated orders. `marketrig [--json] desk events <desk-name-or-id> [--limit n]` prints that desk's operational events newest first — instant, kind, and payload as one-line JSON — over the same keyset listing the desktop pages (per D71, D82). The `trigger` group is `marketrig [--json] trigger <create|list|show|update|enable|disable|delete|firings|firing|invoke> <desk-name-or-id> …` — `create` and `update` take `--name`, `--brief`, `--context`, an optional schedule shape (`--at`, or `--rrule --dtstart --tz`; `--no-schedule` detaches one on `update`), and `--code <file>` with `--suffix`, `--arg`, `--timeout` (defaults: the file's extension, `{script}` alone, the daemon's); the `prompt` group is `marketrig [--json] prompt <list|show> <desk-name-or-id> …`; trigger names resolve through the daemon's per-desk listing, code files are read before the daemon is contacted, and single resources print `field: value` lines in the route's key order (per D79). The `skill` group is `marketrig [--json] skill <put|delete> <desk-name-or-id> …` — `put` takes `--file <SKILL.md>`, read before the daemon is contacted and refused over 64 KiB as a usage error, its name taken from the frontmatter and validated as a path-separator-free identifier (`SKILL_INVALID`); `delete` takes the name. It is the desk's only skill-write path, because OpenViking's own MCP tools refuse the managed skills subtree, and the daemon forwards it under the desk key and refreshes the projection before answering, so the file is on disk when the command returns (§16, per D83). These two commands are the only ones that raise the CLI's shared 10-second ceiling — to 120 seconds, above the daemon's own 90-second write bound, because OpenViking summarizes and embeds a skill through the provider before answering. A trigger's script writing a skill is attributed `TRIGGER` by the same two headers every mutating command sends, with no extra flag. The per-group grammar beyond `desk`, `history`, `trigger`, `prompt`, and `skill`, pagination, and later groups' output shapes are **deferred** to the feature specifications that own each group.
...
Each live desk owns a separate warm xterm parser and renderer. Unicode11Addon selects version 11 before output; bytes remain UTF-8. The parser starts at the adapters' 120×40 and adopts the daemon's dimensions before initial replay. The renderer opens only in a connected element with positive width and height, loads WebglAddon with DOM fallback on initialization failure or context loss, and enables `rescaleOverlappingGlyphs`. Fitting waits for the initial replay write callback; visible mount/resize fits and fully refreshes. Hidden/detached panes retain dimensions and continue parsing. Resizes send actual `term.cols`/`term.rows`, including once after attachment. The well uses a steady accent block caret. On ConPTY, it parks the xterm caret during output bursts and restores it after settle so intermediate redraw positions are not painted; Unix PTYs are unchanged.

The desktop uses terminal identity and absolute byte offsets from §6.5 to skip bytes already queued to its warm parser on one unexpected network reconnect. A changed terminal, an offset gap, or replay ending behind its retained offset stops the viewer with an English continuity-loss notice requesting a reload. While disconnected the parser retains its last dimensions. Normal exit (1000) and takeover/slow-consumer closure (4001) do not automatically reconnect. Pending parser bytes are counted until write callbacks; exceeding 1 MiB stops only the viewer with an English display-buffer notice, without pausing the PTY. Fresh attachment remains best-effort ring replay, not exact screen reconstruction (per D33).
...
Onboarding offers OS-login autostart, enabled by default and configurable; memory stays optional and is set up from Settings, where the operator confirms the two discovered prerequisite paths and the hosted models (§4.4, per D83). Autostart and notifications use Tauri's official plugins with narrowly scoped capabilities; notifications are reserved for actionable conditions — an agent waiting for input, a MarketRig approval, a daemon or runtime failure, a trigger failure — while routine results stay in desk history (per D52).

Concretely (per D82), the shell is the workspace crate `marketrig-desktop`: product name `MarketRig`, main binary `marketrig-desktop` — never `MarketRig`, which on the case-insensitive filesystems both platforms ship is the `marketrig` CLI sidecar's own name — one window, and `marketrigd`, `marketrig`, and `marketrig-mcp` bundled beside it as sidecars so a packaged application resolves them without an installer step. Its whole Rust surface is four commands:

```text
read_endpoint()      -> {port, bearer, daemon_uuid} | null   the parsed runtime/endpoint.json, or nothing
start_daemon()       -> {port, bearer, daemon_uuid}          spawns the sidecar detached; waits for a new daemon UUID in that file within a bounded time, else DAEMON_START_FAILED
set_tray_pending(n)  -> () | Err(text)                       the disabled tray menu line, plus the macOS tray title and the Windows tooltip
exit_app()           -> never returns
```

The daemon is spawned detached — its own session on macOS, its own process group on Windows — because the shell's own crash must not stop it, and the webview alone verifies it (§4.3): read the endpoint, authenticate health, match the UUID, and call `start_daemon` only when that fails. `set_locale` (§4.5) is the shell's one other command: it stores the locale beside the tray items and re-texts them from a label table the shell owns.

Close prevents the window's destruction and hides it, and an exit request the application did not ask for is prevented too, so a hidden window keeps MarketRig alive with nothing on screen. The tray menu is _Open MarketRig_, a disabled pending-approval line whose text `set_tray_pending` sets, and _Quit MarketRig_; _Open_ and a left click unminimize, show, and focus, and the single-instance callback does the same, which is what makes a second launch a focus rather than a second daemon. Quit is one sequence the webview runs from either entry point, the tray's or the window's own confirmed control: `POST /quit`, poll health until it stops answering or a bounded wait elapses, then `exit_app`. Autostart is the official plugin launching with a hidden-start argument, enabled once on a first launch and toggled in Settings, where the plugin's own state is the setting and MarketRig records nothing beside it. The tray starts English and follows the webview's `set_locale` a moment later, once startup has read the setting (§4.5). The shell writes its own bounded log through the official log plugin (§15).
...
Notifications go through the official plugin, one per event and only while the window is hidden or unfocused, for the actionable kinds alone: an approval requested, a session wanting attention, and a failed prompt, desk, trading node, runtime, control plane, or memory child — the last being `OPENVIKING_UNAVAILABLE` — plus a missed trigger. Routine results — fills, deliveries, retains — never notify (per D52). A failed trigger result is not among them, because the delivery event names no firing whose outcome could be read. Click behaviour beyond the platform default stays **deferred** (§18).

The token set is one `@theme` block in the frontend's single stylesheet plus one dark-scheme override and nothing else — the colour roles, the spacing unit, the control radii, the type scale, and the two font stacks D72 fixes — and its concrete values are the desktop feature specification's. Components consume utilities over those tokens and carry no stylesheet and no literal colour. The per-desk terminal socket the desktop attaches to is §6.5's.
...
Migration 2 (per D78) adds the trading tables named in §12.4 and widens `operational_events.kind` with `TRADING_NODE_STARTED` and `TRADING_NODE_FAILED`. SQLite cannot alter a `CHECK`, so a kind widening is a table rebuild — create the widened `STRICT` table, copy, drop, rename, recreate the index — and every later widening repeats that pattern. Migration 3 (per D79) adds `code_snapshots`, `triggers` (with the projection column, a partial unique index on live names, and the partial due index), `firings` (unique per desk, trigger, and occurrence), and `executions` (keyed by firing, `RUNNING | COMPLETE`, the outcome and streams), rebuilds `prompts` for `TRIGGER_RESULT`, `trading_actions` for `source TRIGGER` with `trigger_id` and `firing_id`, and `operational_events` for `TRIGGER_MISSED`. Trigger deletion is soft — `deleted_at_ns`, projection `NULL`, hidden from listings, history readable — because firings reference their trigger. Migration 4 (per D80) adds `desks.selected_runtime` (`codex | claude`, default `codex`), `runtimes` (§4.4, seeded with two `UNDISCOVERED` rows), `native_sessions` keyed by desk and runtime, and `agent_processes` — one row per managed process with runtime, native session, pid, daemon UUID, `started_at_ns`, `ready_at_ns`, `ended_at_ns`, `exit_reason`, and `exit_code`, `ended_at_ns` and `exit_reason` null together, and a partial unique index on `desk_id` over open rows so a desk has at most one live process; rebuilds `prompts` with `state IN ('QUEUED','DELIVERED','FAILED')`, the kinds `ORIENTATION` and `DISCLOSURE`, and the attempt columns `attempted_at_ns`, `resolved_at_ns`, `runtime`, `native_session_id`, `failure_code`, and `disclosed_at_ns` (`resolved_at_ns` set exactly when the state is not `QUEUED`, `failure_code` exactly when it is `FAILED`), carrying every existing row; and widens `operational_events.kind` with `RUNTIME_DISCOVERED`, `RUNTIME_UNAVAILABLE`, `CONTROL_PLANE_STARTED`, `CONTROL_PLANE_LOST`, `SESSION_STARTED`, `SESSION_READY`, `SESSION_POINTER_CHANGED`, `SESSION_ATTENTION`, `SESSION_TURN_ENDED`, `SESSION_INTERRUPTED`, `SESSION_EXITED`, `PROMPT_DELIVERED`, `PROMPT_FAILED`, and `RUNTIME_SWITCHED`. Migration 5 (per D81) adds the one-row `memory_child` and `memory_provider` tables (§4.4) and widens `operational_events.kind` with `MEMORY_CONFIGURED`, `MEMORY_STARTED`, `MEMORY_LOST`, `MEMORY_UNAVAILABLE`, `MEMORY_RETAINED`, and `MEMORY_RECALLED`; the memory child's live state and pid are held in memory only and read not-started after every start, and it needs no recovery step of its own because it is a `children.json` record the first step already reaps. Migration 6 (per D82) adds the one-row `installation_settings` (§4.4), rebuilds `code_snapshots` and `trading_actions` with `approval` and `decided_at_ns` — backfilling every existing row `ALWAYS_ALLOW` with `decided_at_ns = created_at_ns`, and dropping the snapshot's `approved_at_ns`, which the trigger resource keeps deriving — and widens `operational_events.kind` with `POLICY_CHANGED`, `APPROVAL_REQUESTED`, and `APPROVAL_DECIDED`. Migration 7 (per D83) drops `memory_child` for the one-row `openviking_setup` (§4.4), deletes every `MEMORY_*` event, and rebuilds `operational_events` without the six `MEMORY_*` kinds and with `OPENVIKING_CONFIGURED`, `OPENVIKING_PROVISIONED`, `OPENVIKING_STARTED`, `OPENVIKING_LOST`, `OPENVIKING_UNAVAILABLE`, `DESK_MEMORY_PROVISIONED`, `SKILLS_PROJECTED`, and `SKILLS_PROJECTION_FAILED`; migration 8 adds the nullable `memory_provider.embedding_dimension` (§4.4). Migration 10 (feature SPEC `event-triggers` §4) rebuilds `triggers` without `source` and with the schedule optional, and rebuilds `firings` with `request_id` and `input`, replacing the table-level occurrence uniqueness with one partial unique index per entry path. Migration 11 (feature SPEC `localization` §1) adds the nullable `installation_settings.locale` with a `CHECK` on `'en'` and `'zh-Hans'`, no rebuild and no backfill. No prior memory content is read, migrated, or preserved, because the product had no released users. The OpenViking child's live state, its root key, and every desk key stay in memory only, so recovery still needs no step of its own beyond the `children.json` reap. Approvals still own no table of their own: those two rebuilt tables are the records, and the listing is one union query over them. A pending row is ordinary SQLite and survives any restart untouched and decidable; an approved order whose outcome is still null after a crash is the same uncertain action R1 already leaves alone.
...
Long-lived children a crashed daemon left running are recorded in `runtime/children.json` when launched and removed when stopped. Recovery's first step terminates any recorded child of a prior daemon whose process is alive and whose command line still carries the recorded arguments, drops every record either way, and reports each outcome — terminated, not running, pid recycled, or discarded — in the recovery event. A record carrying no arguments carries no identity evidence, so it never matches and is never terminated. Windows discards records without the check, because its Job Object has already ended the children — every kind but `trigger-code`, whose child `process-wrap` can leave suspended outside any job (§9), so it takes the check and the plain terminate on both platforms. `runtime/children.json` is removed only after the recovery transaction commits, so a failed commit keeps its records as evidence for the next start. A trigger script is recorded like any other child, as `trigger-code` with its argv, from the instant its pid is known until its execution ends (per D73).
...
-> state the lesson in the session, which the plugin captures
-> improve reusable procedures in desk skills when useful
-> return to Observe
```

- `marketrigd` owns one installation-wide local OpenViking child, started with the daemon from the virtual environment it provisioned itself (§4.4), run in `api_key` mode behind a per-start root key, with skill extraction disabled and no automatic restart. Its workspace, configuration, and redirected home live under `<data root>/openviking/`.
- The child serves every desk; each desk is exactly one OpenViking user under the one `marketrig` account, derived from its UUID and provisioned at every readiness. Neither the CLI nor the agent ever supplies an account or a user.
- The agent's memory operations are the seeded plugins' own MCP tools; capture and recall are the plugins' hooks. MarketRig proxies no memory content, stores none, and adds no memory command of its own — the one write MarketRig carries is a skill (§13.2).
- MarketRig ships the upstream plugins unmodified and changes only their configuration and registration; it never exposes the child's admin routes, root key, or control plane to agents.
- The child's LLM and embedding models are hosted behind one OpenAI-compatible endpoint; no reranker is configured, and the embedding dimension is measured once at provider save (per D18).
- The child receives the provider key and the daemon's root key through its own process environment as `${…}` references, and each desk's key is written plain into that desk's `0600` `ovcli.conf` under the data root — the two recorded exceptions to the credential boundary (§4.4, per D49, D83). No secret reaches a runtime process environment or a workspace file.
- Each desk has its own `AGENTS.md` carrying the loop and the ownership boundary as always-loaded context, and one canonical skill set — OpenViking's — exposed to both runtimes through the projection within that desk only (§5.1).
- MarketRig seeds an improvement skill into the desk's user and then the agent owns and may evolve it. The agent chooses what to remember, search, write, or delete.
- MarketRig queues Evaluate for every realized-P&L event and never automatically writes a memory or mutates a skill's content. It requires no reflection cadence and no special reflection session (per D22).
- Authoritative reality always stays in MarketRig and sandbox state (per D17).
- A memory child that cannot start, or a provider it refuses, is reported as an explicitly unavailable memory capability. It never blocks daemon startup, native sessions, triggers, or paper trading, and MarketRig never silently substitutes another memory system or model.

The concrete shape (per D83; the full record is the OpenViking feature SPEC §1–§6):

**Setup** is two operator-named absolute paths validated by running them — Python exactly `3.12`, Node major `>= 22` — and one private environment at `<data root>/openviking/venv`, replaced rather than repaired on every provisioning and installed with the bundled `uv` (`--offline --no-index --find-links`, its cache under the data root) from the bundled per-platform wheel directory, which carries that pinned uv binary in the same lockfile, so a missing wheel is a `PROVISION_FAILED` and never a network fetch (§4.4). Each platform's lockfile is generated on that platform, because pip evaluates environment markers against the host when it downloads the set.

**The child** starts with the daemon (§4.6) and on Retry: a loopback port bound and released, a 32-byte hex root key minted per start and held in memory only, `<data root>/openviking/ov.conf` rendered `0600` with both secrets as `${…}` references its loader expands, and `openviking-server` spawned under §9's containment primitive with the data root as its working directory, its `HOME` — and on Windows `USERPROFILE` and `LOCALAPPDATA` — redirected under `<data root>/openviking/`, `PYTHONUTF8=1`, and both output streams captured into one 4 KiB in-memory tail that is never parsed and never logged; the pid is recorded in `runtime/children.json`. Readiness is the first `GET /ready` answering `200`, polled every 500 ms within 120 seconds, and appends `OPENVIKING_STARTED`; because `/ready` embeds once against the provider, a provider the endpoint refuses never reaches ready and its error is the loss's reason. The child exiting or that deadline passing appends `OPENVIKING_LOST`, sets the row `UNAVAILABLE` with the last output line, appends `OPENVIKING_UNAVAILABLE`, and drops the desk keys; there is no automatic restart, and `POST /openviking/retry` runs the start again. Startup does not wait for readiness: desks, triggers, trading, and activation are served while the child is `STARTING`, and a launch with no desk key carries `OPENVIKING_MEMORY_ENABLED=0` and no registration. Every stop — application quit, reprovisioning, or a Retry finding a live process — is `SIGTERM`, up to three seconds, then the group kill; on Windows the plain kill.

**Tenancy.** At every readiness, and behind the creation of a desk while the child is ready, the daemon ensures the account `marketrig` and the user `desk-` plus the desk UUID's hex, then obtains that user's key from OpenViking's own seeded-key route under one installation secret, `openviking_seed`, held in the credential store and never rotated. The key is therefore deterministic in the seed, so a restart or a Retry hands a live session the same key it already holds; it is kept in daemon memory per desk and appears in no SQLite row, log line, event, prompt, or CLI output. `DESK_MEMORY_PROVISIONED` is appended on the first success per daemon start.

**The plugins.** OpenViking's Claude Code and Codex memory plugins are vendored unmodified at one pinned commit, copied at desk creation into `<workspace>/.marketrig/plugins/`, and reconciled byte for byte at startup (§5.1). Registration rides the launch files the daemon already writes: for Claude Code, the plugin's hooks merged into that launch's `settings.json` in exec form with the validated Node path and its own timeouts, and the `openviking` MCP server in that launch's `mcp.json`; for Codex, `[mcp_servers.openviking-memory]` in the workspace's `config.toml` and the plugin's hooks written to `<workspace>/.codex/hooks.json`, whose `command` is one string, always beside its `commandWindows` override (Codex requires the former and rejects the file without it), launched with the flag that trusts the hooks the daemon itself wrote. Credentials are the plugins' own per-desk `ovcli.conf`, written `0600` at every launch under `<data root>/openviking/plugin/<desk-id>/` and preferred by both plugins' resolvers over any environment credential; seven path-only variables name it and keep the plugins' state, pending queue, and workspace registry under the data root while pointing their config file at an absent path so `~/.openviking/` is never read. Those variables ride the runtime process environment for Claude Code, and for Codex are prefixed onto each hook command and set on the MCP entry's `env` table, because the Codex app-server runs every desk's hooks and MCP servers with no per-desk environment (per D83).

**Skills and the projection.** A desk's skills are canonical in its OpenViking user. Before every activation, after every `SESSION_TURN_ENDED`, and once before a skill write answers, the daemon lists the user's skills, fetches each with its content and auxiliary files, and rewrites `.agents/skills/` to match exactly — a temporary tree swapped into place, absent skills removed, files `0444` and directories `0555` on macOS and read-only attributes on Windows — with the `.claude/skills` link untouched. An invalid skill name is skipped and named; a child that is not ready or a fetch that fails leaves the previous tree, appends `SKILLS_PROJECTION_FAILED`, and never blocks the activation; success appends `SKILLS_PROJECTED`. Refreshes coalesce, one in flight per desk with one queued. A direct edit to a projected file is refused by the filesystem and the constitution says so. The write path is `marketrig skill put|delete` (§13.2), which the daemon forwards under the desk key with a 90-second write bound and projects before answering; `503 OPENVIKING_UNAVAILABLE` when there is no key or no ready child, `502 OPENVIKING_REJECTED` carrying OpenViking's own message.

**The seeded texts** — the `AGENTS.md` constitution and the `desk-improvement` skill — live in the daemon's source tree under `crates/marketrigd/seed/`, with the desk name as their one substitution, so a wording change is a file change and the checks compare them byte for byte. The constitution names the loop and the ownership boundary, all three planes and their surfaces, the paper environment's limits, the evaluate-and-learn step with realized P&L as the reward, the memory tools, and the projection rule that a skill is written through `marketrig skill` and never edited on disk. The constitution is written once at creation and never rewritten (§5.2); the skill is uploaded into the desk's user behind the creation request, skipped when one of that name exists, and reaches the workspace through the projection.
...
**The gate** is unattended and deterministic. The daemon, the CLI, the database, the NautilusTrader sandbox, and every public surface are real; the sandbox is driven by the milestone's deterministic stand-in quote source where one exists (per D76) and by live public data where it does not; and the runtime CLIs and the memory child are the harness's stand-ins — `runtime-standin`, which passes both runtimes' probes and speaks exactly the protocol subset the adapters consume, scripted per launch, and `openviking-standin`, which the daemon starts, stops, and reprovisions in the child's place and which serves the OpenViking subset the daemon consumes. The chain therefore proves MarketRig's mechanics with no model, no key, no first-run dialog, no interpreter, and no operator.

**The experiment** is the operator-attended run of the same chain on the real Codex CLI or Claude Code and a real OpenViking child behind a real hosted endpoint, once per release candidate per platform-and-runtime cell, whose bundle is archived as the evidence that a real agent closes the loop. Every native session start is operator-attended through a terminal relay so runtime dialogs are answered by a person.
...
- trigger invocation adds no helper and one attended scenario (per the amended D34; slice 015, frozen 2026-09-12). T1–T5 run after A6 with no runtime registered, so every prompt they queue resolves `RUNTIME_UNAVAILABLE` and delivery stays G28's evidence: T1 invokes a schedule-less trigger once, replays the request id as `DUPLICATE`, and fires a burst of distinct requests once each; T2 reads the input in the code's firing document and in the result prompt; T3 consumes a one-off through the invocation door and through the scheduler and shows either door disables it; T4 proves a refusal — disabled, deleted, pending code, or oversized input — buffers no firing, execution, or prompt; and T5 shows a request id answers its original firing after a clean stop and a restart. E8 runs last in each attended cell: with the console as the desk's terminal, the harness invokes `review-research` with a three-line text through `marketrig trigger invoke`, and the daemon delivers the result carrying the request id and the input to the session it launched, the replay answers `DUPLICATE` with no second firing or prompt, and what the agent made of the lines ends inconclusive (feature SPEC `event-triggers` §7);
- localization adds one scenario, L1 after T5 (slice 016, frozen 2026-09-14): on one daemon with no runtime registered, under `zh-Hans` and then under `en`, a desk with its seeds, one invoked code-bearing firing, and one closed round trip are produced, and every agent-facing artifact — the seeded `AGENTS.md` and skills, the firing document, the `TRIGGER_RESULT` and `EVALUATION` rows, `marketrig --help`, `trigger --help`, and `desk show` plain and `--json`, both not-found answers, the OpenAPI document, the adapter's resource and tool listings, and the daemon's own log lines — is byte-identical pair by pair after identifiers and instants are substituted (feature SPEC `localization` §4, §6.1);
...
Concretely (per D82), CI is two jobs on both platforms: `checks` runs rustfmt, Clippy with `-D warnings`, and the whole workspace's tests including the gate, and `frontend` regenerates the REST client from `marketrigd --openapi`, fails on any diff in the committed document or client, and then runs Prettier, ESLint, `vue-tsc`, and Vitest. The packaged smoke is `pnpm smoke`: WebdriverIO with its Tauri service driving the bundled application through the shell's own embedded WebDriver server, which is compiled in behind a Cargo feature the smoke build alone enables — a shipped build never carries it, because that server listens on loopback without authentication. It refuses to start unless its own wipe variable is set, under which it first deletes the per-user data root, both log directories, and `~/.marketrig`; it is the one leg that touches the real root, so it is operator-run once per platform and never in CI, and each run leaves its WebdriverIO report and the shell's log in `target/acceptance/smoke-<platform>-<stamp>/`. It registers `runtime-standin` by explicit path through the Settings tab so a terminal can attach, and drives the REST routes with the bearer it reads from the real endpoint file. It runs in `zh-Hans`: its first step changes the language through the Settings select and reads the Chinese heading, its second types a Chinese line into the well and reads the pty's echo, and the operator confirms the tray, one notification, and the input method by eye (feature SPEC `localization` §6.2).
...
- exact packaging layout, launch shims, code signing and notarization, per-user CLI PATH registration, and recovery from an autostart registration the operating system refuses;
- the exact crate selections and pins still named as candidates in §9 and §10, each confirmed against current documentation at plan time and pinned in the workspace manifest; §4.3's web framework, §15's SQLite binding and logging facade, and §4.4's credential-store binding are pinned already (per D77, D81);
- domain-owned SQLite table details for each milestone; approvals own no table of their own, because the code-snapshot and trading-action rows are the records;
- CLI invocation grammar beyond the `desk`, `history`, `trigger`, `prompt`, and `skill` groups, pagination, and later groups' output shapes;
- public trigger ingress, connectors, daemon-supervised producers, and payload filtering beyond direct invocation (§8.2);
- OpenBB research as a whole — its supervised child, connectors, secret references, and credential flows — deferred past MVP on scope (per D9); the Kraken crypto environment of D74 likewise, past MVP (per D84);
- exact localized copy for surfaces later milestones add, renderer-loss recovery details, and detailed interaction design;
- notification click behaviour beyond the platform default, and a notification for a trigger result that failed, whose delivery event names no firing to read the outcome from (§14);
- OpenViking tuning beyond the fields §16 writes, hosted reranker configuration, embedding-model change, and memory erasure and relocation workflows;
...
- retention and pruning of firings and their invocation inputs, with the other records above;
...
- validating Windows' `LongPathsEnabled` before provisioning: `volcengine-python-sdk` ships wheel members past the legacy 260-character `MAX_PATH`, so a deep data root fails inside `pip install` instead of being told so up front (per D83, §16);
- a daemon-visible turn end on Codex, which exposes none, so the turn-end projection refresh fires only on Claude Code's `Stop` hook and a Codex desk's projection waits for its next activation (per D83, §16);
- the Claude Code memory plugin's hard-coded `~/.openviking/last_inject.md` and probe cache, the one thing MarketRig's environment fencing does not redirect, because the vendored plugins are unmodified (per D83, §16).
