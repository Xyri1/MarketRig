...
*Decision basis: per D3, D11, D13, D14, D18, D25, D42, D44, D47, D48, D49, D52, D58, D59, D66, D68, D70, D77, D80, D81, and D82.*
...
The memory child's launch target and the memory provider are two more installation resources (per D81). `memory_child` is one row with `state IN ('UNCONFIGURED','AVAILABLE','UNAVAILABLE')`, the exact launcher path, `validated_at_ns`, and a failure code and message: read through `GET /memory`, resolved through `POST /memory/discover {executable}` — an absolute path validated by `<executable> --help` within 10 seconds requiring `HINDSIGHT_API_PORT` in its standard output, failing `NOT_FOUND`, `CAPABILITY_MISSING`, or `PROBE_FAILED` — and cleared of a `CHILD_FAILED` through `POST /memory/retry`. `memory_provider` is one row with `base_url`, `llm_model`, `embedding_model`, an opaque key reference, and the embedding-lock instant, written by `PUT /memory/provider {base_url, api_key?, llm_model, embedding_model}`; an omitted `api_key` keeps the stored one, every answer carries `api_key_present` and never a key, and a provider change stops a live memory child so the next operation starts it with the new environment (§16).
...
For Hindsight's hosted models the user sets one OpenAI-compatible base URL, API key, LLM model, and embedding model; the same endpoint and key serve both, and no reranking model is configured. SQLite persists the base URL and selected models; the key stays in the credential store. Both model ids are typed by the user; `GET /memory/provider/models` still lists the provider's models live, never persisted or cached, with a fetch failure explicit — `PROVIDER_UNREACHABLE` — rather than a stale list, but no MarketRig surface drives a selector from it. The embedding model locks once Hindsight has initialized data with it: the lock instant is stamped by the first successful retain, after which a `PUT` naming a different embedding model is refused `EMBEDDING_MODEL_LOCKED` (per D18, D81).
...
*Decision basis: per D66, D73, D77, D80, and D81.*
...
6b. re-validate an AVAILABLE memory child (§16); skipped under MARKETRIG_TEST_DATA_ROOT like 6a. The child itself is never started here
...
- MarketRig owns runtime compatibility material and seeds the desk constitution and the initial improvement skill from its own texts, which ship in the daemon's source tree under `crates/marketrigd/seed/` with the desk name substituted (§16, per D81).
...
*Decision basis: per D4, D50, D63, D68, D69, D71, D77, D78, D80, D81, and D82.*
...
*Decision basis: per D22, D23, D36, D38, D45, D46, D51, D71, D73, D77, D78, D79, D80, D81, and D82.*
...
*Decision basis: per D16, D17, D18, D19, D21, D22, D47, D49, D65, and D81.*
...
- `marketrigd` owns one installation-wide local Hindsight instance, run as one supervised loopback child process on the launcher the `memory_child` row names (§4.4) — the bundled interpreter and pinned wheel environment once packaging ships, and until then the one the operator names (per D47, D65) — authenticated with a per-start bearer credential, with its own MCP server and telemetry disabled and one automatic restart. Its embedded database is Hindsight-managed under a MarketRig-named instance.
...
The concrete shape (per D81; the full record is the R4 feature SPEC §2–§5):

**The child** is started lazily, by the first memory operation that finds none live and never by startup or by a status read: a loopback port bound and released, a 32-byte hex bearer minted per start and held in memory only, the launcher spawned under §9's containment primitive with the data root as its working directory and recorded in `runtime/children.json`, its `HOME` — and on Windows `USERPROFILE` and `LOCALAPPDATA` — redirected to `<data root>/hindsight/` so the embedded PostgreSQL installation and the MarketRig-named instance live inside MarketRig's own root on macOS — on Windows `pg0` resolves the profile through the known-folder API and ignores all three variables, so they live under the real `%USERPROFILE%\.pg0\` and every daemon on the machine shares that one cluster, desk isolation resting on the bank name alone (verified 2026-09-04; §18) — its MCP server and telemetry off, one worker, Python in UTF-8 mode (`PYTHONUTF8=1`: the banner is block glyphs, and off a terminal a GBK Windows box otherwise exits on the first one), and both its output streams captured into one 4 KiB in-memory tail that is never parsed and never logged. Readiness is the first `GET /health` answering `200`, polled every 500 ms within 120 seconds, and appends `MEMORY_STARTED`. The child exiting or that deadline passing appends `MEMORY_LOST`; the next operation starts it once more, and a second loss with no readiness between sets the row `UNAVAILABLE` with the child's last output line and holds every operation at `MEMORY_UNAVAILABLE` until Retry (§4.4). A key the provider refuses is not a loss — the child may start healthy and the operation carries the failure. Every stop — application quit, a provider change, or a loss whose child is still alive — is `SIGTERM`, up to three seconds, then the group kill, followed by a `SIGTERM` to the postmaster the instance's `postmaster.pid` names, because the embedded database runs in its own session outside the containment group and Hindsight stops it only from its own handler; on Windows neither signal exists, the plain kill stands, and a cluster a crash leaves behind is **deferred** (§18).

**Banks and operations.** A desk's bank is `desk-` followed by its UUID with the hyphens removed, computed per request, created by Hindsight itself on first use, and present in no response, log line, or event. `GET /desks/{d}/memory` and `POST /desks/{d}/memory/{retain|recall|reflect}` are synchronous pass-throughs carrying the bearer, the agent's content, context, tags, query, and budget verbatim, and retain's attribution metadata — source, desk, and the trigger and firing identifiers when the request carries §12.3's two attribution headers, validated as the order routes validate them. MarketRig adds no tag and no timestamp, stores no content, and appends only `MEMORY_RETAINED` and `MEMORY_RECALLED`, neither of which carries content. Limits are content at most 64 KiB, context at most 4 KiB, query at most 8 KiB, at most 16 tags of at most 64 characters, and a budget of `low`, `mid`, or `high`; the daemon's timeouts are 180 seconds for retain and reflect and 60 for recall. The codes, all inside §4.3's envelope, are `MEMORY_UNCONFIGURED`, `MEMORY_UNAVAILABLE`, `MEMORY_REJECTED`, `MEMORY_TIMEOUT`, and `MEMORY_ERROR`, and every message the daemon lifts out of the child — into a response, an event, a stored failure, or a log line — is redacted first: the stored provider key, and any delimited token beginning with its first eight characters, becomes `<redacted>`, because Hindsight's own error detail quotes the provider, which quotes the key back.

**The seeded texts** — the `AGENTS.md` constitution and `.agents/skills/desk-improvement/SKILL.md` — live in the daemon's source tree under `crates/marketrigd/seed/`, with the desk name as their one substitution, so a wording change is a file change and the checks compare them byte for byte. The constitution names the loop and the ownership boundary, both planes and their surfaces, the paper environment's limits, the evaluate-and-learn step with realized P&L as the reward, and memory and skills as the agent's own; the skill describes one way to carry that out. Both are written once at creation and never rewritten (§5.2).

## 17. Verification

*Decision basis: per D60, D61, D67, D75, D76, D77, D78, D80, D81, and D82.*
...
- the harness ships a third helper binary, `memory-standin`, which the gate registers as the memory launcher by explicit path (per D81). It answers the `--help` probe with the marker, then serves `/health`, retain, recall, and reflect on the exact paths and field names the daemon consumes, behind the per-start bearer, over an in-memory per-bank store with substring recall, and takes its readiness delay, its scripted exit, and a rejected retain from the `memory` object of the same `MARKETRIG_STANDIN_SCRIPT` file; invoked as `memory-standin --models <port>` it serves a scripted model list and is the gate's provider stand-in, started by the gate itself rather than by the daemon. G33–G37 extend the chain after G32 — configuration and secrets, two desks and one lesson including a trigger's own retain, the loop closing where the `EVALUATION` prompt arrives as the stand-in session's own `INPUT` line and the harness then performs the agent's retain and skill edit through public surfaces alone, a later session recalling that lesson and reading the improved skill through `.claude/skills` on the other runtime, and Hindsight lost, restarted once, then unavailable while trading, triggers, and activation continue, ending with a hard kill whose child the next start reaps; E5 is the attended scenario in which a real session closes the same loop on real Hindsight behind a real hosted endpoint, skipped with evidence when the operator's environment names no launcher or no provider;
- the desktop milestone adds no helper and no attended scenario, because its evidence is mechanical (per D82). G38–G41 extend the chain after G37 — the policy resource with `STEER` refused and the event tail read live, reconnected gaplessly, and closed `4408` on a consumer that stops reading; trigger-code approval, where a pending trigger never becomes due through enable, disable, or an elapsed occurrence and a denied one leaves no firing, execution, or prompt; paper-order approval, where a pending order reaches no sandbox, replays idempotently, refuses its cancel, and after approval fills, closes a cycle, and queues its evaluation exactly as an ungated one; and the three sockets under a foreign origin, a wrong first frame, and a hard kill after which both pending records are still decidable and a reconnecting client receives the successor's recovery unit. Because the installed default gates trigger code (per D70), G21's prologue and the experiment's E3 setup put that policy on **Always allow** before their first code-bearing trigger, and G38 restores the default.
...
- CLI invocation grammar beyond the `desk`, `history`, `trigger`, `prompt`, and `memory` groups, pagination, and later groups' output shapes;
...
- stopping the memory child's embedded database on Windows, where no signal reaches the child: the plain kill stands, and a PostgreSQL a crash or an ungraceful stop leaves running waits for a later `pg0 stop` (per D81); that cluster also lives under the real profile rather than the data root, shared by every data root on the machine, because `pg0` ignores the redirected `HOME`, `USERPROFILE`, and `LOCALAPPDATA` there (§16);
- a supported way to reach that database at all — the daemon knows the instance's path only because it chose the `HOME` the child runs under, and neither Hindsight nor pg0 offers a query or a stop the daemon could call instead (per D81).
