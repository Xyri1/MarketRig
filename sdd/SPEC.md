...
OpenBB research is deferred past MVP on scope (per D9): MarketRig bundles no research provider, and research is the agent's own through its shell, tools, and workspace. When it arrives it is informational only, never a writer of trading state, reached only through `marketrig`, and hosted as its own supervised child beside the memory child's.
...
**Setup** is two operator-named absolute paths validated by running them — Python exactly `3.12`, Node major `>= 22` — and one private environment at `<data root>/openviking/venv`, replaced rather than repaired on every provisioning and installed with `pip --no-index --find-links` from the bundled per-platform wheel directory, so a missing wheel is a `PROVISION_FAILED` and never a network fetch (§4.4). Each platform's lockfile is generated on that platform, because pip evaluates environment markers against the host.
...
- replaying a `LIMIT` order's history: its stored events carry no `OrderSubmitted` or `OrderAccepted`, so `GET /desks/{id}/history/orders` cannot reconstruct it and omits it — a trading-plane defect found by E6 on 2026-09-08, owed a gate regression ([ROADMAP deferred work](ROADMAP.md#deferred--post-mvp)).
