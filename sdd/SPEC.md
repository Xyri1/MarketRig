...
There is no layered override hierarchy, no live settings file, no generic key/value configuration layer, and no environment-variable override hierarchy. Typed non-secret configuration is authoritative in the installation-wide SQLite database and changes only through `marketrigd`. The `~/.marketrig/` directory holds default desk workspaces and no authoritative configuration or daemon state.
...
*Decision basis: per D24, D25, D27, D28, D31, D32, and D69.*
...
Per-runtime mechanics — the Codex app-server connection and its remote-TUI attachment, the Claude Channel router and its per-session bridge, the Claude hook set and its per-launch settings file, and every launch argument beyond the resume pointers above — are **deferred** to the runtime feature specification. Two components are fixed regardless: the Channel bridge is a Rust binary built from MarketRig's own Cargo workspace, owning no durable state and no business logic (per D36, D53), and the terminal substrate is §6.5's.
...
Whether macOS needs a separate single-threaded spawn helper is decided at plan time against the chosen crate and the daemon's own threading model, because fork safety is a property of the host process rather than of the product (per D31). Ring size and the resize and shutdown timings are **deferred** to the runtime feature specification.

## 7. Activation

*Decision basis: per D22, D28, and D36.*
...
*Decision basis: per D35 and D41.*
...
The firing document's fields, the source and output size bounds, the timeout range and default, artifact storage and preview mechanics, and the environment variable names are **deferred** to the trigger feature specification.

## 10. Scheduled-trigger semantics

*Decision basis: per D37 and D40.*
...
Scheduled occurrence identity is the trigger identity plus its scheduled instant, and the firing table enforces that uniqueness, so a duplicate wake cannot execute or deliver the same occurrence twice. Storage layout and transaction scenarios are **deferred** to the scheduling feature specification.

## 11. Daemon prompt delivery

*Decision basis: per D36, D70, and D71.*
...
Trigger-result input carries only the trigger reference, the firing-time brief and context snapshot, and the raw result or artifact reference. Evaluation input carries a stable realized-P&L history reference with enough order, fill, position, fee, and provenance identity for the agent to query supporting history. Operational queue status and installation policy are queryable but are never injected into a prompt.
...
The REST surface, the per-desk terminal socket, the live-event socket, the right panel's composition, the shell commands, and the design tokens' concrete values are **deferred** to the desktop feature specification.
...
**The gate** is unattended and deterministic. The daemon, the CLI, the database, the NautilusTrader sandbox, and every public surface are real; the sandbox is driven by the milestone's deterministic stand-in quote source where one exists (per D76) and by live public data where it does not; and the runtime CLIs and the memory child are the harness's stand-ins — a stub runtime control plane with a scripted agent that performs the workspace addendum's steps on the prompts it receives, the stub runtime probes, and a fake memory child. The chain therefore proves MarketRig's mechanics with no model, no key, no first-run dialog, and no operator.
