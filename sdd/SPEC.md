...
For Hindsight's hosted models the user sets one OpenAI-compatible base URL, API key, LLM model, and embedding model; the same endpoint and key serve both, and no reranking model is configured. SQLite persists the base URL and selected models; the key stays in the credential store. The provider's model list is fetched live whenever the selector opens through `GET /memory/provider/models`, never persisted or cached, and a fetch failure is explicit — `PROVIDER_UNREACHABLE` — rather than a stale list. The embedding model locks once Hindsight has initialized data with it: the lock instant is stamped by the first successful retain, after which a `PUT` naming a different embedding model is refused `EMBEDDING_MODEL_LOCKED` (per D18, D81).
