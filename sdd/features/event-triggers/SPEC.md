...
All on the stand-in feed with the `trigger-code` binary, no runtime registered; every assertion is a route read or one read-only SQLite query.

- **T1 — invoke, replay, burst.** A code-free recurring trigger with no schedule; `marketrig trigger invoke --request-id r1 --input hello` → exit 0, `outcome: ACCEPTED`, a firing with `request_id r1` and the input, one `TRIGGER_RESULT` prompt `QUEUED` whose payload carries `invocation.input`; the same command again → `DUPLICATE`, the same firing id; after 2 s still one firing and one prompt; then 50 distinct request ids in a loop → 50 firings in acceptance order and 50 queued prompts, none refused; an `--input-file` of 200,000 bytes is accepted and the prompt payload carries `input_bytes` without `input`, while `trigger firing` returns it whole. The listing of a second desk's identically named trigger has no firing.
...
- **T3 — one-off through both doors.** (a) A one-off `--at` 5 s ahead, invoked at once → accepted, `enabled: false`, `next_occurrence_ns` absent; after 8 s still one firing and no `TRIGGER_MISSED`. (b) A one-off `--at` 2 s ahead fires on schedule; `invoke` with `r-a` → `TRIGGER_DISABLED` naming that firing; `enable`; `r-a` again → `ACCEPTED`, because the refusal stored nothing; `r-a` a third time → `DUPLICATE`; the trigger reads `enabled: false` again with two firings, one scheduled and one invoked. (c) A one-off `--at` 2 s ahead, `disable` at once, wait 3 s, `enable` → `invoke` is `TRIGGER_ELAPSED`; `update --no-schedule` → `ACCEPTED`. (d) A recurring every-minute trigger invoked twice → two firings and `next_occurrence_ns` unchanged across both.
...
- **T5 — duplicates survive restart.** Invoke `r-restart` on a code-free trigger; `POST /quit`; restart; the same request id → `DUPLICATE` with the same firing id; exactly one prompt, still `QUEUED`, and the `RECOVERY` event lists nothing lost.
