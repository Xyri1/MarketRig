...
4. set paper orders to *Require approval*; `POST /desks/{d}/orders` with the bearer → the Approvals tab shows one item and the tray count is 1; *Approve* → the action reads `APPROVED` in history and, while the real feed quotes the instrument, the Desk tab shows the position (off-hours the sandbox's own outcome is `MARKET_PRICE_UNAVAILABLE`, which the smoke accepts as the authoritative answer); a second order → *Deny* → `DENIED` in history actions;
