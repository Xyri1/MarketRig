...
Vitest with Vue Test Utils and jsdom, against a fake `fetch` and a fake `WebSocket`: `useDaemon` verifies UUID match and falls back to `start_daemon` once; `useEvents` sends the cursor on reconnect, dispatches by kind, and calls only refetches; `useTerminal` checks distinct per-desk instances, measurable opening, hidden dimensions, renderer fallback/context loss, replay completion before fitting, replay deduplication and gap rejection, ConPTY metadata, pending-write limits, and disposal on `SESSION_EXITED`; a Node test uses real xterm and Unicode11 to check byte-split UTF-8, emoji, CJK, and combining widths before opening; `useApprovals` counts per desk and calls `set_tray_pending`; the Approvals tab renders both kinds and disables its buttons while a decision is in flight; the policy `Select` renders *Steer* disabled; the catalog check finds no bare template string; the drift check is CI's `git diff --exit-code src/client` after `pnpm generate`.
