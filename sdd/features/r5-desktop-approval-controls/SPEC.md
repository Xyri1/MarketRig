...
### 4.4 Origin and first-frame authentication on every WebSocket, and CORS on REST

The allowlist is exactly `tauri://localhost` (macOS production), `http://tauri.localhost` (Windows production), and `http://localhost:5173` (Vite's dev server, `build.devUrl`); a present `Origin` outside it is `403 ORIGIN_REFUSED` before any upgrade, on `/events`, `/desks/{d}/terminal`, and `/desks/{d}/channel` alike. Requests carrying no `Origin` pass to the bearer check. `/desks/{d}/terminal` with an `Authorization` header behaves exactly as R3 §3 specifies; without one it upgrades, requires `{"bearer": …}` as its first text frame within 5 s (`4401`), then answers its pre-attachment checks as close codes — `4404 DESK_NOT_FOUND`, `4409 NO_LIVE_SESSION` — and only then takes the attachment generation, so an unauthenticated or refused connection never supersedes a live viewer. `/desks/{d}/channel` stays header-only. Close-code reasons carry the same SCREAMING_SNAKE code as text. The same allowlist governs REST, because the webview reaches this loopback API cross-origin with an `Authorization` header: an `OPTIONS` preflight from an allowed origin is answered `204` before the bearer check — no credential, `Access-Control-Allow-Origin` naming that origin, `Access-Control-Allow-Methods: GET, POST, PUT, PATCH, DELETE`, `Access-Control-Allow-Headers: authorization, content-type`, `Access-Control-Max-Age: 600`, `Vary: Origin` — every other answer to an allowed origin carries the echoed origin and `Vary: Origin`, a present `Origin` outside the allowlist is `403 ORIGIN_REFUSED` before the route, and a request with no `Origin` is untouched and gains no header.
...
- **Restart is a row.** The daemon restarts; the client's reconnect with its last cursor receives the successor's recovery unit first and nothing from before the kill — the `SESSION_EXITED` row of every session the restart lost, then the `RECOVERY` that names them, all at that unit's one instant (R3 §6.3).
...
- **Refused origin.** `Origin: https://example.com` → `403 ORIGIN_REFUSED`, no upgrade, on all three sockets and on REST alike; `Origin: tauri://localhost` with no header and a wrong first frame → upgrade then `4401`; a header-authenticated terminal client (the harness) is untouched by the new path.

## 5. The shell (R5-6)

`src-tauri/` — crate `marketrig-desktop`, product name `MarketRig`, main binary `marketrig-desktop` (never `MarketRig`: on the case-insensitive filesystems both platforms ship, that name is the `marketrig` CLI sidecar's, and whichever is written last wins), identifier `dev.marketrig.desktop`, one window `main` (1280×800 default, shrunk to the monitor's work area and centred at setup so no row hides under a taskbar, 960×600 minimum, `visible: true` unless launched with `--hidden`), `build.devUrl = http://localhost:5173`, `build.frontendDist = ../dist`, `bundle.externalBin = ["binaries/marketrigd", "binaries/marketrig", "binaries/marketrig-mcp"]` (Tauri's sidecar convention: the source files carry the `-<target-triple>` suffix the CLI strips when bundling, and `pnpm tauri build` is preceded by `cargo build --release` and a copy step in `package.json`), `bundle.targets` `dmg` on macOS and `nsis` on Windows (`tauri_build::build()` resolves `externalBin` on every build, so `build.rs` writes an empty placeholder for each missing sidecar and the copy step overwrites it), and the capability `main.json` granting only `core:default`, `core:window:allow-hide`, `core:window:allow-show`, `core:window:allow-set-focus`, `core:window:allow-unminimize`, `notification:default`, `autostart:default`, and `log:default` — the four commands need no entry, because a Tauri 2 capability gates plugin commands only and `generate_handler!` commands on the app itself are always reachable from its windows.
...
Terminal wiring: `@xterm/xterm/css/xterm.css` is imported once in `main.ts`. Each live desk owns a separate warm xterm parser and renderer. Unicode11Addon selects version 11 before output; bytes remain UTF-8. The parser starts at the adapters' 120×40 and adopts the daemon's dimensions before initial replay. The renderer opens only in a connected element with positive width and height, loads WebglAddon with DOM fallback on initialization failure or context loss, and enables `rescaleOverlappingGlyphs`. Fitting waits for the initial replay write callback; visible mount/resize fits and fully refreshes. Hidden/detached panes retain dimensions and continue parsing. Resizes send actual `term.cols`/`term.rows`, including once after attachment. The well uses a steady accent block caret (`cursorBlink: false`). On ConPTY, it appends a local DECTCEM hide only after chunks that end with a visible cursor (Codex frames often end with a show at a transient CUP cell) and restores the caret ~250 ms after settle or on key input; plain idle output is left alone so the steady composer caret remains. Unix PTYs are unchanged.
...
`onData` sends UTF-8 input. The `exited` frame writes one dim machine line; `SESSION_EXITED` disposes the pane. A reload discovers the selected desk through SessionHeader. CSS theme tokens are passed directly to xterm: background from `--color-well`, foreground from `--color-state-idle`, cursor from `--color-accent`. Pins verified against npm and installed source on 2026-09-06: xterm 6.0.0, fit 0.11.0, webgl 0.19.0, unicode11 0.9.0. No mutation updates lists optimistically; controls await the daemon and refetch (per D72).
...
4. set paper orders to *Require approval*; `POST /desks/{d}/orders` with the bearer → the Approvals tab shows one item and the desk row's pending badge reads 1 (the tray's count is the same `useApprovals` total, and a Tauri command has no DOM for WebDriver to read); *Approve* → the action reads `APPROVED` in history and the smoke waits for the sandbox's own outcome: `FILLED` → the Desk tab shows the position; `DENIED` (off-hours, or a node that has not yet observed the instrument, since a node prices only from observations made after it started and the daemon's quote cache is no proof of that) → accepted as the authoritative answer; a second order → *Deny* → `DENIED` in history actions;
...
6. `api` — origin allowlist on the three sockets and on REST (preflight answered `204` without the bearer, the echoed origin on an answer, a foreign origin `403 ORIGIN_REFUSED`, no `Origin` untouched), header path unchanged, first-frame terminal path with `4404`/`4409`, `--openapi` prints a document every route appears in (`paths` count equals the router's) and exits `0` without a data root.
