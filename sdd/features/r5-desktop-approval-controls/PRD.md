...
6. **The frontend** (per D29, D33, D55–D59, D62, D72): the Vue 3 application over the client generated from the daemon's own OpenAPI document, the three panels, one warm xterm.js presentation per live terminal, the events-driven refetch model, the approval and notification surfaces, and the one token set with its concrete values.
