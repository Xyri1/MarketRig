...
`exec::spawn(command) -> Contained` is the one primitive: on macOS it wraps the tokio command in `process-wrap`'s `ProcessSession` — `setsid` before exec, so the child leads a new session and process group — and `terminate` sends `SIGKILL` to the group; on Windows it wraps in `JobObject` and `KillOnDrop`, and then puts the child in MarketRig's own kill-on-close Job Object as well, because `process-wrap =9.1.0` never sets `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` (its spawn takes the wrapper map out before `JobObject` looks for `KillOnDrop`; observed 2026-09-03: a `sleep 30` outlived its hard-killed daemon by 52 minutes) — `terminate` ends the crate's job, and the daemon's own death closes MarketRig's, which ends the tree. The terminal manager's PTY children (R3) share that job helper. `process-wrap` spawns `CREATE_SUSPENDED` and resumes the child only after its own job assignment, so a daemon that dies inside that window leaves a child that never ran and never will (one thread, suspended, still holding its executable open — observed on Windows in G26, 2026-09-07: a `sleep 30` runner survived its hard-killed daemon and broke the next build with `LNK1104`). Every trigger-code child is therefore recorded in `runtime/children.json` as `trigger-code` with its argv, from the instant the pid is known, and forgotten the moment the execution ends however it ends (root §15), so the next daemon's first recovery step resolves a stranded one — on Windows this kind alone takes the identity check instead of the discard, because `TerminateProcess` ends a suspended process no Job Object holds, and on macOS the same check kills a script that outlived its daemon in its own session. Ceiling: the record is what closes the gap, not the spawn, so a daemon that dies between `CreateProcess` and the record still strands a child, and a stranded child of either kind lives until the *next* start rather than being cleaned up at the kill. Assigning MarketRig's job before the resume — a spawn path of MarketRig's own — is the upgrade if the recorded pid ever proves not enough. Both platforms expose the same handle (`id`, piped stdin/stdout/stderr, `wait`, `terminate`) and nothing else, so R3's app-server and R4's memory child reuse it unchanged (per D73). A run's execution row is settled by recovery as `DAEMON_LOST` either way, and no script is ever resumed.
...
Nullable fields are omitted when null, as R0's resources do — `x: null | {…}` above names the union, and the key is absent (never a literal `null`) in the null case, at the top level and inside `code` and `execution` alike. A create body's unknown keys are ignored; a patch naming no recognized key is `TRIGGER_INVALID` like the empty object; a name outside the desk-name grammar is `TRIGGER_INVALID`; `delete` touches `deleted_at_ns` and the projection only, never `revision` or `updated_at_ns`. A **Firing** is its row plus `execution: null | { "state", "daemon_uuid", "outcome", "exit_code", "error", "executable", "stdout", "stderr", "stdout_bytes", "stderr_bytes", "stdout_truncated", "stderr_truncated", "started_at_ns", "finished_at_ns" }`; the per-trigger listing carries the same object without `stdout` and `stderr`. A **Prompt** is `{ "id", "desk_id", "kind", "state", "created_at_ns" }` in the listing and adds `payload` on the single read (root §11.1).
...
- `exec::running_child_is_recorded` — a running execution's pid is a `trigger-code` record in `runtime/children.json` carrying its argv, and the record is gone once the execution ends (§4.5); `daemon::reap_identity_check` covers the reap of a `trigger-code` record whose process is dead or foreign.
