...
One NautilusTrader **cash** account with netting positions per desk, created with the desk's node and seeded exactly once: **100,000 USD, 1,000,000 HKD, 1,000,000 CNY**. No conversion between balances: an instrument trades against its own currency. Under the hood the account is per venue — NautilusTrader routes one venue to one execution client and resolves each account by its issuer — so the desk's book is four venue accounts seeded in their market's currency, a market's seed split evenly across its venues (the CN seed is 500,000 CNY at each of XSHG and XSHE); desk-wide totals match the figures above, no balance crosses venues, and a sufficiency refusal is per-venue with the sandbox's own reason saying so. Wiring follows root §12.1's normative rules verbatim: sandbox execution client through the builder's simulated-client entry point; the fee model configured **explicitly** — never implicit — to charge each market's declared per-side rate, whether that rides the instrument type's own fee fields or an explicit MarketRig fee model (settled at slice time against the pinned crates, per R1-4); the data-event sender taken on the node thread; one venue routed to one execution client per node.
...
Submission is synchronous through the sandbox: the route answers after the sandbox accepts (and, for a marketable order, fills) or refuses. Orders on a desk that is not `READY` answer `DESK_NOT_READY`. "Marketable" here means matched **in full** on arrival: the sandbox names a pending fill only for a fully matched order, so that is the only fill the answer waits for. A partially marketable order answers `ACCEPTED` or `PARTIALLY_FILLED` depending on when the read lands; its fill is authoritative in the order's own state and its history a moment later, never lost.
...
Migration 2 also widens `operational_events.kind` with `TRADING_NODE_STARTED` and `TRADING_NODE_FAILED` — as a table rebuild (create the widened `STRICT` table, copy, drop, rename, recreate the index), since SQLite cannot alter a `CHECK`; the pattern every later kind widening repeats. Every sandbox event lands in its event table, and the `book_snapshots` row is rewritten, inside one unit with whatever else that event implies; order and position listings are query projections over these tables, never stored ones (R1-5). An order's stored chain is the node's **own** order — the event list NautilusTrader keeps on it — not the stream that woke the capture: the sandbox can apply an event to the cached order without ever publishing it, and a chain that omits such an event cannot be replayed back into an order. Capture therefore appends whichever of the order's events are not yet stored — matched by NautilusTrader's own `event_id` inside the payload, never by position, so a chain stored under an earlier rule is completed rather than duplicated and one trade id can never land two `fills` rows — and falls back to the received event alone when the node holds no such order. They are still NautilusTrader's payloads, verbatim (per D38). §7's `history/orders` rebuilds every order from that chain, so every captured order is listed. MarketRig holds no `Position` objects outside the node (R1-5, settling root §12.4's open question).
...
The ActionRecord is the `trading_actions` row's JSON: `action_id`, `id`, `kind`, `created_at_ns`, and the outcome (for a submit, the order's current projection including `client_order_id`). History routes return complete newest-first lists; pagination stays deferred (root §18). `history/orders` rebuilds each order from its stored chain (§5) rather than from a stored projection, so an order whose chain cannot be replayed is logged at error and omitted from the answer rather than failing the route; §5's capture rule is what keeps that list empty.
...
- **Resources**, enumerated concretely in `resources/list`, five entries, JSON bodies identical to the §7 routes they proxy: `marketrig://desk/<name>/quotes`, `…/book`, `…/positions`, `…/orders`, `…/instruments`. A read calls the daemon at read time; the adapter caches nothing and says so on the wire, declaring a zero cache TTL on every read — the protocol's 2026 revision gives clients an on-by-default response cache, and silence would leave freshness to a client default.
- **Tools**: `submit_order` (the §4.2 body, minus desk) and `cancel_order` (`client_order_id`, `action_id`). The daemon validates everything; the advertised schema is documentation (per D4). One adapter-side check is the exception, because `client_order_id` addresses the cancel route's path: the adapter requires it to be a single safe path segment before building the URL — a path-injection guard on the authenticated daemon surface, answering the standard tool error with `ORDER_INVALID` — and forwards everything else about it untouched. A daemon refusal, an unreachable daemon, or a failed verification surfaces as a structured tool error whose text carries the §4.3 envelope's code and message — explicit, never retried.
...
- **G14 — resting, cancel, and replay.** A limit buy below market rests; the same `action_id` resubmitted answers `200` with the original record and creates no second order; cancel with a fresh `action_id` removes it; a limit buy that is marketable on arrival partially fills, stores the acceptance the bus never carried, cancels, and is listed by `history/orders` with no replay failure logged; the desk is flat again; cancel of an unknown id answers `ORDER_NOT_FOUND`.
...
- **G17 — restoration.** With a resting limit order standing: `POST /quit`, restart, first market-plane read restores the book — the order is open under its **original** `client_order_id`, the durable book snapshot's positions and balances are undisturbed (the live before/after comparison is `trade::snapshot_restores_book`'s layer — no layer restates another, root §17), history rows are untouched, and quotes are `UNAVAILABLE` until the stand-in is polled again.
- **G18 — feed honesty.** The stand-in answers 429 on the first seven attempts and succeeds on the eighth → observation accepted (the retry bound's last attempt); a poll of nine straight 429s → no new observation, health `DEGRADED`; recovered to `LIVE`, the stand-in then goes dark → health `DEGRADED`, last observation standing with growing age; a never-observed instrument reads `UNAVAILABLE` with no price fields.
...
- **E1 / E2 — real runtimes on the market plane.** For Codex CLI and Claude Code in turn, operator-attended, real Yahoo, MCP registration by hand: the session reads the desk's quote resource twice and sees a fresh value, then submits and cancels a paper order through the typed tools; the harness verifies by side effects (the `trading_actions` rows and the resting order's lifecycle). The quote reads leave no side effect at all — observations are never persisted — so that half is inconclusive by construction; agent-behavior aspects end inconclusive rather than failed (root §17).
...
- `trade::limit_order_history_replays` — a limit order marketable on arrival and a resting one: both chains are the node's own, and `history/orders` lists both, newest first.
- `trade::capture_repairs_a_chain_stored_without_its_acceptance` — a chain stored without the acceptance the bus never carried is completed by the next capture, with no event and no fill stored twice.
