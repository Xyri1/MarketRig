...
**Decision:** The equity `DataClient` polls Yahoo's chart endpoint per subscribed instrument: once at subscription regardless of market phase, then every 10 seconds while that instrument's calendar phase is `OPEN`, and not otherwise. HTTP 429 is retried up to 8 attempts 400 ms apart (the counts the data spike verified, per D76); exhaustion leaves the last accepted observation standing with degraded health and is never a silent provider substitution. The endpoint layer is the community `yahoo_finance_api` crate (per D76, candidate pinned at plan time); the base URL is compiled in and overridable only through the acceptance seam (R1-9), never by configuration.
...
**Rationale:** Ten seconds is far inside Yahoo's observed tolerance yet fast enough that a fill against a moving book is meaningful; polling only during `OPEN` is the cheapest 429 hygiene there is. Promising a delay we cannot observe would be an assumption dressed as a contract — D76 ordered the delay characteristics verified, and what verification found is that only timestamps are honest.
...
**Decision:** Every mutating trading command carries a caller-supplied `action_id` (1–64 characters, `[a-z0-9-]`), unique per desk: the daemon records it in `trading_actions` before touching the sandbox, and a repeat of the same `action_id` returns the original action's record without acting (per D64, root §12.3). The daemon gains the §7 REST routes; the CLI gains exactly `marketrig [--json] history <orders|fills|cycles> <desk-name-or-id>`, newest first, over durable records only. New error codes, append-only per D68: `INSTRUMENT_UNKNOWN` 404, `ORDER_NOT_FOUND` 404, `ORDER_INVALID` 400, `INSUFFICIENT_BALANCE` 409, `INSUFFICIENT_POSITION` 409, `MARKET_UNAVAILABLE` 503, `DESK_NOT_READY` 409.
