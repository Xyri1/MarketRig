...
CN only; US and HK retain their existing contracts. This proposal changes CN's phase-independent execution, GTC lifetime, and form-only validation boundary. AE-1–AE-10 record the slice’s amendments; root SDD remains the delivered contract until exit checks pass and changes are merged back. [FEASIBILITY.md](FEASIBILITY.md) records the completed spike and known implementation defects; [RESEARCH.md](RESEARCH.md) holds supporting evidence.
...
E7 runs in one attended sitting, at any wall-clock hour, and stages both its trading days through the seam above rather than sending the operator away for a day (per AE-10). The cell seeds every node at 10:00 Asia/Shanghai on the buy day — the trading weekday before the sell day, which is itself the most recent weekday whose 09:30 Shanghai has passed, so the provider already holds that day's own bar. The session reads real research and a band under the disclosed provider-reference assumptions, buys, observes the lock, and receives the same-day sell refusal. The harness then advances every node to 10:00 on the sell day with `PUT /test/clock`, which unlocks the lot exactly as the rollover does in A1 and re-establishes the day against the provider's real calendar and that day's bar — the seam's lifted cadence gate is what keeps the leg polling for it after the real close — and asks the same session for the sell: the native cycle closes and evaluation is queued. Only the trading dates are staged — the calendar, the bar and every snapshot are the real service's on both legs — and because both legs trade against the same day's quotes minutes apart, the realized figure is recorded as a harness artifact. Outside real Shanghai hours that snapshot is the day's last and does not move, so both legs are MARKET orders (AE-9 fills them against the latest usable snapshot; a LIMIT would wait for volume growth that only a trading market produces). Both staged days step over the weekend; a holiday neither step can see leaves the instrument `UNAVAILABLE`, which the cell prints at the top of its instructions and records inconclusive, so the operator stops rather than driving a sitting that cannot buy. Hold over a corporate-action date only with the accounting limitation explicitly recorded; do not attribute omitted dividend effects to strategy performance.
