...
`runtime/launch/<desk-id>/settings.json` (0600): `{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"\"<marketrig>\" --desk <desk-id> session hook"}]}],"Notification":[…same…],"Stop":[…same…]}}`. `<marketrig>` is the CLI beside the daemon binary, written with forward slashes and quoted because Claude runs hook commands through bash on every platform, where a backslashed path collapses into one word (found by the Windows E5 cell, 2026-09-04); when it is missing the settings file is omitted and the launch carries no hooks (per D69). Both files are deleted when the process row closes.
