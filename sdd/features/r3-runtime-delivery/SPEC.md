...
`GET /desks/{desk_id}/terminal` (bearer as every route; the `Sec-WebSocket-Protocol` header is not used) answers `404 DESK_NOT_FOUND`, `409 NO_LIVE_SESSION` when no terminal exists, and `400 VALIDATION` when the request is not a WebSocket upgrade — all three before any attachment is taken, so a request that never upgrades cannot supersede a live one — else upgrades. Frames: binary = bytes; text = `{"resize":{"cols":n,"rows":n}}` from the client, `{"exited":{"reason":…,"code":…}}` from the server followed by close `1000`. The ring is replayed as one binary frame before live bytes.
...
`runtime/launch/<desk-id>/settings.json` (0600): `{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"<marketrig>","args":["--desk","<desk-id>","session","hook"]}]}],"Notification":[…same…],"Stop":[…same…]}}`. `<marketrig>` is the CLI beside the daemon binary; the hook is Claude's exec form — with `args` present the executable is spawned directly, no shell on any platform (Claude Code hooks reference, checked 2026-09-04) — because the shell form runs under Git Bash on Windows too, where the backslashed path collapsed into one word (found by the Windows E5 cell, 2026-09-04); when it is missing the settings file is omitted and the launch carries no hooks (per D69). Both files are deleted when the process row closes.
