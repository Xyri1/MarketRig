...
The child runs under the R2 containment primitive (`ProcessSession` on macOS, a Job Object on Windows), its standard error captured into a 4 KiB tail the daemon keeps in memory; nothing it prints is logged at `info`. Readiness appends `MEMORY_STARTED {pid}`; live state `NOT_STARTED → STARTING → READY`. A memory operation arriving during `STARTING` waits for readiness inside its own timeout (§4.3).

### 2.3 Loss, restart, and stop

The child exiting, or the health deadline passing, ends the attempt: the process tree is terminated, the record dropped, `MEMORY_LOST {pid, exit_code, stderr_tail_last_line}` appended, live state `LOST`. The next operation starts the child again once; if that attempt is lost too before a readiness in between, the row becomes `UNAVAILABLE CHILD_FAILED` with the last standard-error line as `failure_message`, `MEMORY_UNAVAILABLE` is appended, and every operation answers `MEMORY_UNAVAILABLE` until `POST /memory/retry`. A provider change (§3) stops a live child so the next operation starts it with the new environment. Quit stops the child after every terminal and the Codex app-server, within the existing 5 s shutdown bound.
...
- **Wrong key.** Hindsight's startup model verification fails; the child exits before `/health` answers; the operation answers `MEMORY_UNAVAILABLE` after the second loss with the child's own last line, and `marketrig memory status` shows it.
...
1. `memory::discover` — explicit path, the `--help` marker, the three probe failure codes, step 6b skipped under the seam.
...
4. `memory::ops` — bank derivation, the three request mappings field for field, attribution metadata from the headers (`ATTRIBUTION_INVALID` on a bad firing), the limits, every error code in §4.3, `MEMORY_RETAINED` and `MEMORY_RECALLED` payloads, no content in any event.
