...
Update `crates/marketrig-acceptance/EXPERIMENT.md` and E7 for a real-provider buy, same-day T+1 refusal, and — on the trading day the harness stages through the controlled clock (per AE-10) — the sell producing the native cycle and queued evaluation, all in one sitting. Preserve the unknown-delay, corporate-action and staged-calendar limitations in the evidence. Run attended cells only with the operator and prerequisites present; do not claim success from a skipped or inconclusive cell.
...
- Revised attended E7 closes on all four platform/runtime cells (macOS/Windows × Codex/Claude), each in one sitting on the staged trading day, with one queued evaluation. Record bundle paths and actual status here; no auto-created schedules or unattended substitutes.
...
- Step 5: `Harness::standin_clock`/`advance_clock`, stand-in scripting for `prev_price`, cumulative volume, bar date and a hidden instrument; H2's trading half moved to the controlled clock with the same-day refusal and the next-day sell; G16 and O9's sandbox refusals moved to AAPL; A1–A6 appended after H4; E7 procedure and driver revised to a buy, the same-day refusal and a later-session close. Amended 2026-09-11 by AE-10: E7 reaches that close inside one sitting, seeding its nodes on the buy day and moving them to the sell day after the refusal, so `Harness::partial` is gone with the two-day protocol. Amended again the same day so the sitting runs at any wall-clock hour: the sell day is the most recent weekday whose 09:30 Shanghai has passed and the buy day the weekday before it, and the controlled-clock seam lifts the `CN` cadence gate in `node::poll_cn` the way the HiThink stand-in does, because otherwise a staged daemon polls nothing after the real close and the moved-on day never re-establishes. Both legs are MARKET orders, since an after-hours snapshot never grows its volume.
...
- Windows E7 (2026-09-11, commit 1c42cb3 plus the key-validation fix below): both cells complete in one sitting on staged days 20260910 → 20260911 — `target/acceptance/experiment-e7-codex-1789114428` (Codex CLI 0.154.0) and `experiment-e7-claude-1789115853` (Claude Code 2.1.268): provider `AVAILABLE`, the `CN` observation `provider: hithink`, `calendar: WEEKDAY`, null source time; 100 × `600519.XSHG` MARKET buy filled, the same-day sell refused `ORDER_INVALID` by T+1, the sell on the moved-on day filled, cycle realized −76.50 CNY (two commissions), evaluation queued; key in no file of the bundle. Two earlier Codex attempts that day failed at `PUT /research/hithink` with `PROVIDER_UNREACHABLE` "builder error": the operator's key carried a control byte, which reqwest reports that way; `Hithink::put` now refuses such a key as `VALIDATION` before the request. The Claude session, left with only the ORIENTATION prompt, twice opened an unrelated `000001.XSHE` position on its own before the operator's steps arrived; the harness ignores it, so the cell is complete, but the driver should hand the session its first step at readiness.
...
Outstanding before freeze: the module checks and the extended gate on Windows; the attended E7 cells on the two macOS pairs (Windows done above), one sitting each, with the close on the staged trading day and one queued evaluation; root merge of the AE amendments into SPEC §12, §13, §17 and DECISIONS, and the ROADMAP/AGENTS.md refresh, which wait for those checks.
