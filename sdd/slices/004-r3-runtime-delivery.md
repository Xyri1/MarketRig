...
- **Recovery registration:** `daemon::recover` gains the `sessions` step after `executions`, calling `session::recovery_step(tx, daemon_uuid) -> (Vec<Value>, Vec<Value>)` for `sessions_lost` and `prompts_unknown`.
...
- **Spike S, first day of C25, before adapter code:** on Codex 0.152.1 with a real app-server and a `--remote` TUI in a `portable-pty` terminal — (S1) `-c mcp_servers.marketrig.*` on the TUI command line reaches the remote thread's MCP configuration (`mcpServerStatus/list` names it); (S2) a `turn/start` from a second connection on the TUI's thread routes `item/*/requestApproval` to the TUI, not to the second connection; (S3) `--ws-auth capability-token` is accepted on a loopback listener. Each answer is recorded in R3-3's ponytail note, and a negative replaces the mechanism with its named fallback in the same change.
...
C25 and C26 run in parallel after C24, each against an `Adapter` trait C24 lands beside the terminal manager (`spawn`, `deliver`, `interrupt`, `exit`, readiness and exit events over one `mpsc`); C27 follows both; C28 is last. C24's checks drive a plain `cat`/`cmd` child, not a runtime.
