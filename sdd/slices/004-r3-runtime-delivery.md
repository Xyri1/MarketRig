...
| `futures-util` | `=0.3.34` | marketrigd | already in the graph (tokio-tungstenite, nautilus); the `StreamExt`/`SinkExt` the split app-server socket is read and written through. |
...
- **Adapter seam (C26):** `Adapter::deliver` takes `(desk_id, prompt_id, kind, text)`: Claude's channel frame carries the meta the bridge republishes (feature SPEC §5.3), and Codex ignores the two extra arguments. The Claude adapter and the channel route share one `claude::Channels` registry held in `ApiState`, which also carries the dispatcher's `AdapterEvents` sender — the route, not the adapter, sees a bridge connect, and that connection is readiness.
...
- **Spike S addendum, run 2026-09-03 before C25's adapter code:** (S4) the daemon cannot pre-create a new session's thread — `codex resume` on a thread that has taken no turn fails with `no rollout found for thread id` — and it does not need to: (S5) a plain `--remote` launch broadcasts a non-`ephemeral` `thread/started` within a second of spawn, carrying the workspace `cwd` and `status: idle` inline, with no `thread/status/changed` after it, while a resume broadcasts only `thread/status/changed` `notLoaded` → `idle`. Readiness is therefore "the pointer's thread is `idle`", tracked from both broadcasts; feature SPEC §4.2 and R3-3 carry it.
- **Test targets:** G27–G32 extend `--test gate` after G26; E4 joins `--test experiment` as a fourth attended test gated by `MARKETRIG_EXPERIMENT`, serialized after E3 in the same invocation. `cargo test -p marketrig-acceptance --test standin` is R3-8's own check of the stand-in runtime, run with the module checks. `MARKETRIG_STANDIN_SCRIPT` is read only by `runtime-standin`.
