...
- **Startup and shutdown glue (`lib.rs::serve`):** two `Arc<Notify>` (scheduler wake, executor wake) and the shutdown `watch` are created in `serve`, handed to `api::ApiState` and to the two tasks, which start after the listener is live; shutdown awaits `exec::stop` (terminate groups, persist `QUIT`) inside the existing 5-second deadline before `startup.shutdown()`.
...
| C18 | Schedules and the scheduler | §2, §3 — `schedule::Schedule` (`parse`, `next_after`, `count_between`), the acceptance unit, the task, `TRIGGER_MISSED` | `schedule::form_rejected`, `schedule::dst_gap_skipped_overlap_earlier`, `schedule::projection_from_anchor`, `schedule::accept_or_miss`, `schedule::duplicate_wake_no_second_firing`, `schedule::wake_and_recheck` | C17 |
| C19 | Trigger definitions, firings, prompts, REST | §4.1 snapshots and fingerprint, §8 in full, the wake signal after mutations | `exec::fingerprint_stable` (lives with the snapshot code in `trigger.rs`; keep the name), `api::trigger_codes` | C18 |
