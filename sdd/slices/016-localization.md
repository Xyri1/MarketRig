...
**Status:** Frozen (2026-09-14). Steps 1–5 implemented on macOS 2026-09-14 on branch `localization` (5caa4a7, cb87252, cacce96, c24f4ba, 3fbc071, 942578c, 8803365); the gate is green through L1 on macOS (`gate-1789368276`) and on Windows CI; CI is green on both platforms at 30ad007 (run 34824531177; the run on 8803365 hit a pre-existing race in the stand-in's scripted-exit test, closed in 30ad007); the packaged `zh-Hans` smoke passed on macOS (`smoke-darwin-2026-09-14T08-24-35-763`) with the operator's eye confirmations. Merged back the same day: root SPEC §4.5, §5, §15, §17; D68 amended in place with the LZ summary; ROADMAP's R7 item records the delivery; AGENTS.md its state and the smoke sentence; no new product D number.
...
Implementation (2026-09-14, branch `localization`): steps 1–5 implemented. Module checks green on macOS: `store::locale_migration_applies`, `policy::locale_resource`, `api::openapi_lists_locale`; `cli_utf8_output` (two tests) in `marketrig`; `tray_labels_cover_both_locales_and_counts` and `parse_locale_takes_the_two_shipped_tags_only` in `marketrig-desktop` (`set_locale` needs an `AppHandle` no unit test can build, so its refusal is checked through `parse_locale`, which every call goes through); `pnpm check` 79 Vitest tests including the catalog parity, `locale.test.ts`, the Settings select, the notification title, and the App startup checks; `pnpm generate` clean after commit. Gate green through L1 on macOS: `gate-1789368276`, 974 s, 481 observations, all fifteen artifact pairs byte-identical (the seeded `AGENTS.md` and skills, the firing document, the `TRIGGER_RESULT` and `EVALUATION` rows, four CLI outputs, both not-found answers, the OpenAPI document, the MCP resource and tool listings, the daemon's own log lines). Drift folded into the feature SPEC in the same commits: the `INPUT n:` claim (§5, §6.2), the reload-free smoke path through the Language select (§6.2), the vue-i18n `Legacy` type parameter (§2.4), the native select's snap-back mechanic (§2.5), the uncollectable `ORIENTATION` text and the log-file source of item 8 (§4, §6.1). Three L1 lessons: a node prices market orders only from observations made after it started, and the quote `sequence` counts per daemon and instrument, so each half ticks the feed after its node starts and waits for the sequence to advance; no tick between the two fills, so the realized figure is `0.00` in both halves and no fill timing enters; the daemon's stderr is empty by design, so item 8 reads the log root. CI (runs on 642d410 and 253ed0a): macOS `checks` and both `frontend` jobs green; the Windows gate green (1075 s) but the Windows `checks` job failed because the `marketrig-desktop` unit-test exe died `0xc0000139 STATUS_ENTRYPOINT_NOT_FOUND` before any test ran — the new test built `TrayState::default()`, a type holding a `MenuItem`, which linked Tauri's Win32 stack (muda, `comctl32!TaskDialogIndirect`) into a unittest exe that carries no Common Controls v6 manifest (tauri-build's manifest reaches bin targets only). Diagnosed on a Windows 11 machine by `dumpbin /imports` against master's exe; fixed by asserting on a `DEFAULT_LOCALE` const instead, so no unit test touches a Tauri type. CI green on both platforms at 942578c (run 34818963254: both `checks` jobs including the gate through L1, both `frontend` jobs). The packaged `zh-Hans` smoke passed on macOS on the fifth run (`target/acceptance/smoke-darwin-2026-09-14T08-24-35-763`, five steps, the stored locale `zh-Hans` after the run). The four failed runs taught, and the same commit records: the wheel set needed its pinned `uv` binary fetched beside it; WebKit's WebDriver delivers no CJK through key actions and a `<select>` in WKWebView opens a native menu a WebDriver option click reaches only sometimes, so the smoke dispatches the `insertText` input event and the select's `change` from the page (feature SPEC §5, §6.2); a Language change made while startup's read or write is in flight was overwritten by it, fixed in `App.vue` with an `App` check (feature SPEC §2.2); and R5's step 4 chose its branch from the daemon's quote cache, which is no proof the desk's node has observed, so it now waits for the order's own outcome (R5 feature SPEC §7.3 corrected in place). The operator confirmed by eye on the packaged application (2026-09-14): the three tray labels in Simplified Chinese, one notification with a Chinese title, and the OS input-method popup with correctly rendered glyphs in the well (LZ-7). The Windows smoke is not a freeze condition (as slices 014 and 015) and is recorded here when run.
