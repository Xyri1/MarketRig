...
cargo test -p marketrig-acceptance --test gate     # the gate alone (G1–G32, then O1–O10, then H1–H4, then A1–A6, ~30 min)
...
# A cell runs E1 or E2, then E3, E4, E6, and E7, serialized: they share the operator.
# E6 needs a real Python 3.12, Node >= 22, the locked wheel set, and a provider, and skips with evidence without them; E7 needs the real HiThink key and skips with evidence without it.
