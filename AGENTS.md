...
- The root SDD set (`sdd/PRD.md`, `sdd/DECISIONS.md` through D80, `sdd/SPEC.md`, `sdd/ROADMAP.md`) was founded fresh on 2026-09-01 and is the only product truth. Milestones R0–R3 are delivered (slices `sdd/slices/001-r0-foundation.md`, `002-r1-equity-paper-trading.md`, `003-r2-scheduled-triggers.md`, and `004-r3-runtime-delivery.md`, all frozen; per D77, D78, D79, and D80): the Cargo workspace, `marketrigd`, `marketrig`, `marketrig-mcp`, the equity paper-trading plane, scheduled triggers with code execution, runtime discovery, the terminal manager, the Codex and Claude Code adapters with dispatcher-driven delivery, and the acceptance gate and experiment exist. Milestone R4 (memory, skills, and the closed loop) is next: its feature folder `sdd/features/r4-memory-skills-loop/` is design complete (PRD, DECISIONS R4-1 … R4-6, SPEC, 2026-09-04) and its implementation plan is the active slice `sdd/slices/005-r4-memory-skills-loop.md` (Spike H, chunks C29–C33; implementation not started).
...
cargo test -p marketrig-acceptance --test gate     # the gate alone (G1–G32, ~10 min)

# The attended experiment, one cell at a time; unset it and every cell skips.
# A cell runs E1 or E2, then E3, then E4, serialized: they share the operator.
...
The operator's procedure for an attended cell — timing, project-scoped adapter registration, driving E1/E2, E3, and E4 (whose console *is* the desk's terminal, and which registers nothing because MarketRig launches the runtime itself), reading the bundle — is `crates/marketrig-acceptance/EXPERIMENT.md`.

Both acceptance modes build and drive the real binaries themselves — `marketrigd`, `marketrig`, `marketrig-mcp`, `trigger-code`, the acceptance-only helper every code-bearing trigger names as `argv[0]` (per `sdd/features/r2-scheduled-triggers/SPEC.md` §10.1), and `runtime-standin`, the gate's stand-in runtime, which G27–G32 register by explicit path and script through the one JSON file `MARKETRIG_STANDIN_SCRIPT` names on the daemon's environment (per `sdd/features/r3-runtime-delivery/SPEC.md` §9.1) — and write an evidence bundle to `target/acceptance/gate-<stamp>/`, `experiment-<cell>-<stamp>/`, `experiment-e3-<cell>-<stamp>/`, or `experiment-e4-<cell>-<stamp>/` (`MARKETRIG_ACCEPTANCE_OUT` overrides, so leave it unset for an experiment cell, whose three scenarios would then share one directory): `observations.jsonl` one JSON line per step, `marketrigd-N.stderr` per daemon, the relocated `data/`, `desks/`, and `logs/`, the gate's `scripts/`, and the experiment's `instructions.txt`, `instructions-e3.txt`, or `instructions-e4.txt`. The gate runs its own stand-in feed on loopback; the experiment polls real Yahoo and prints what the operator must do by hand. CI (`.github/workflows/ci.yml`) runs the three checks on macOS and Windows; the experiment stays operator-run.
